...
class ucclient_db {
	...
	function connect($dbhost, $dbuser, $dbpw, $dbname = '', $dbcharset = '', $pconnect = 0, $tablepre = '', $time = 0) {
		...
		if ($this->version() > '4.1') {
			if ($dbcharset) {
				mysqli_query( $this->conn,"SET character_set_connection=" . $dbcharset . ", character_set_results=" . $dbcharset . ", character_set_client=binary");
			}
	...
	function error() {
		return (($this->conn) ? mysqli_error($this->conn) : mysqli_error());
	}

	function errno() {
		return intval(($this->conn) ? mysqli_errno($this->conn) : mysqli_errno());
	}
