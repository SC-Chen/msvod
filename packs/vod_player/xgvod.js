...
function Showhtml(){
       ...
       if(player==''){
	      parent.document.getElementById('msvod_vodplay').src='http://static.xigua.com/installpage.html?msvod';
       }else{
...
function PlayerNt(){
	   ...
	   if (navigator.plugins) {
		var install = true;
		    ...
		    if(!install){
				    var player = '<div style="width:100%; height:'+parent.ms_height+'px;overflow:hidden;position:relative"><object  width="100%" height="'+height+'" type="application/xgyingshi-activex" progid="xgax.player.1" param_URL="'+unescape(url)+'" param_NextCacheUrl="'+unescape(xpurl)+'" param_LastWebPage="'+surl+'" param_NextWebPage="'+xurl+'" param_Autoplay="1" id="xiguaPlayer" wmode="opaque" name="xiguaPlayer"></object></div>';
		        }
	    }
        return player;
}
...
function PlayerIOS(){
	...
	location.href = url+"|http://'+window.location.host + parent.ms_root+'packs/vod_player/loading.html|"+xurl;
}
