...
class Topic extends msvod_Controller {
	...
	public function index()
	{
            ...
            $sql_string = "SELECT id,name,pic,hits,tid,yid,singerid,addtime FROM ".MS_SqlPrefix."video_topic where 1=1";
			if($yid==1){
	...
	public function init()
	{
            ...
            if($ac=='zt'){ //锁定
                  ...
                  $edit['tid']=$sid;
		          $str=($sid==1)?'<a title="'.L('plub_03').'" href="javascript:get_cmd(\''.site_url('video/admin/topic/init').'?sid=0\',\'tj\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/ok.gif" /></a>':'<a title="点击推荐" href="javascript:get_cmd(\''.site_url('video/admin/topic/init').'?sid=1\',\'tj\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/no.gif" /></a>';
			}
	...
	public function save()
	{
            ...
            admin_msg(L('plub_70'),site_url('video/admin/topic'),'ok');  //操作成功
	}
	...
	public function del()
	{
			...
			$result=$this->db->query("SELECT id,pic FROM ".MS_SqlPrefix."video_topic where id in(".$idss.")")->result();
			$this->load->library('msup');
			foreach ($result as $row) {
                  if(!empty($row->pic)){
					    $this->msup->del($row->pic,'videotopic'); //删除图片
				  }
				  //删除金币和经验
				  $this->dt($row->id,1);
			}
	...
	public function dt($id,$sid=0)
	{
			...
			if($dt){
				  ...
				  if($sid>0){ //删除
				      ...
				      $add['uidb']=0;
				      $add['name']='专题被删除';
				      $add['neir']='您的专题《'.$dt->name.'》被删除，系统同时扣除您'.User_Cion_Del.'个金币，'.User_Jinyan_Del.'个经验';
				      $add['addtime']=time();
