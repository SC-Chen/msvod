 ...
 * @Dtime:2014-12-08
 */
class Saomiao extends msvod_Controller {
			    ...
			    foreach ($showarr as $t) {
				    if (is_dir($t['server_path'])) {
					        $dirs[] = array(
						       ...
						       'link' => site_url('video/admin/saomiao/yps')."?dir=".$dir."&hz=".$hz."&path=".$path."/".$t['name'],
						       'rklink' => site_url('video/admin/saomiao/ruku')."?dir=".$dir."&id=".$path."/".$t['name'],
					        );
				    } else {
				  ...
				  if(is_dir($file)){  //文件夹
						 ...
						 if(!empty($strs)){
                             foreach ($strs as $value) {  
                                  if(!empty($value)){
                                        $video=addslashes($value);
										$video=str_replace($dir,"",$video);
										$exts = strtolower(trim(strrchr($video, '.'), '.'));
										$name=explode("/",$video);
	...
	public function ftp()
	{
		    if(FTP_Sm_Server=='' || FTP_Sm_Name==''){
                  admin_msg(L('plub_94'),site_url('video/admin/saomiao/ftp_config'),'no');
			}
	...
	public function ftpsave()
	{
            ...
            if (!write_file(MSVOD.'lib/Ms_FtpSm.php', $strs)){
                      ...
                      admin_msg(L('plub_96'),site_url('video/admin/saomiao/ftp'));
            }
	}
	...
	public function ftpruku()
	{
            ...
            if(!empty($arrs)){
                ...
                foreach ($arrs as $file) {  
                        $file=get_bm($file);
                        $video=addslashes($file);
				        if(strpos($video,$path) === FALSE){
                            $video=$path.$video;
						}elseif(substr($video,0,1)=='.'){
	...
	public function djinfo($dir)
	{
		...
		$look = $this->mp3file->get_metadata($dir);
        if(!empty($look['Filesize']) && !empty($look['Bitrate']) && !empty($look['Length mm:ss'])){
      	    return array("dx"=>formatsize($look['Filesize']),"yz"=>$look['Bitrate']." Kbps","sc"=>$look['Length mm:ss']);
   	    }else{
      		return false;
   	    }
	}
