...
if(urlArray[j].indexOf('qvideo://')!=-1){
var t=urlArray[j].split("$");
t[1]=t[1].replace(/[\u3016\u3010\u005b]\w+(\.\w+)*[\u005d\u3011\u3017]/ig,'').replace(/(qvideo:\/\/\d+\|\w+\|)(.+)$/i,'$1['+(window.siteurl || document.domain)+']$2').replace(/(^\s*)|(\s*$)/ig,'');
urlArray[j]=t.join("$");
}
newStr+=urlArray[j]+"\n";
}
...
Popsboxif('地址采集',480,176,parent.web_path+parent.web_self+'/video/admin/video/caiji?id='+id+'&sid='+sid);
}
