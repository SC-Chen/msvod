...
function wap_player(n) { 
     ...
     var video= $('#player_'+n).attr('data-str');
     arr = video.split("==msvod==");
     name=arr[0];
     did=arr[1];
     url=msvod_path+'index.php/video/play/url/wap/'+did+'?msvod.mp3';
     audio = $("#audio")[0];
     audio.src = url;
     $('#songname').html(name);
     pid=n;
...
function wap_player_one() { 
     url=msvod_path+'index.php/video/play/url/wap/'+mp3_id+'?msvod.mp3';
     audio = $("#audio")[0];
     audio.src = url;
     pu.downloadlrc(0);
     pu.PlayLrc(0);
...
function play_next(){
    var videocount=$(".video_list li").length; 
    if(audio){
        audio.pause();
    }
