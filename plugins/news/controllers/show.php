...
//获取数据
$row=$this->MsdjDB->get_row_arr('news','*',$id);
...
if($id==0) exit();
//获取数据
$row=$this->MsdjDB->get_row_arr('news','name,uid,cid,yid,hid,id,vip,level,cion,content',$id);
if(!$row || $row['yid']>0 || $row['hid']>0){
...
if($row['vip']>$rowu['zid']){
exit("alert('你的会员级别不能阅读该小说内容，请先升级！');");
}
}
...
//判断是否下载过
$did=$id;
$rowd=$this->db->query("SELECT id,addtime FROM ".MS_SqlPrefix."news_look where did='".$did."' and uid='".$_SESSION['msvod__id']."'")->row_array();
if($rowd){
$down=1; //数据已经存在
...
exit("alert('阅读小说需要".$row['cion']."个金币，您的金币不足无法显示内容，请先充值！');");
}else{
//扣币
...
$add['cion']=$row['cion'];
$add['addtime']=time();
$this->MsdjDB->get_insert('news_look',$add);
}
}
