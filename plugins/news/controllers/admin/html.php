...
class Html extends msvod_Controller {
	...
	public function index()
	{
		    ...
		    $Mark_Text=$this->MsdjTpl->plub_index('news','index.html',true);
	...
	public function topic_save()
	{
            ...
            if($datacount==0){
				 ...
				 $sqlstr="select count(*) as count from ".MS_SqlPrefix."news_topic where yid=0";
            ...
            for($i=$start;$i<=$pagejs;$i++){
				  ...
				  $Mark_Text=$this->MsdjTpl->plub_list(array(),1,'lists',$page,'',true,'topic.html','topic','','小说专题');
	...
	public function topicshow_save()
	{
            ...
            if($datacount==0){
				 $sqlstr="select count(*) as count from ".MS_SqlPrefix."news_topic where yid=0 ".$str;
                 $rows=$this->db->query($sqlstr)->result_array();
				 $datacount=$rows[0]['count'];
            ...
            $sql_string="select * from ".MS_SqlPrefix."news_topic where yid=0 ".$str." order by id desc";
	...
	public function type_save()
	{
            ...
            if($ac=='all' && $nums==0){
				   $cid=array();
                   $query = $this->db->query("SELECT id FROM ".MS_SqlPrefix."news_list where yid=0 order by xid asc"); 
            ...
            if($datacount==0){
				 ...
				 $sqlstr="select count(*) as count from ".MS_SqlPrefix."news where yid=0 and hid=0 and cid IN (". getChild($id).")";
                 $rows=$this->db->query($sqlstr)->result_array();
				 $datacount=$rows[0]['count'];
    	         $pagejs = ceil($datacount/$pagesize);
			}
            ...
            for($i=$start;$i<=$pagejs;$i++){
				  ...
				  $Mark_Text=$this->MsdjTpl->plub_list($row,$id,$type,$i,$ids,true,$skins,'lists','news',$row['name'],$row['name']);
			      write_file(FCPATH.$Htmllink,$Mark_Text);
				  echo "<a target='_blank' href='".$Htmllinks."'>&nbsp;&nbsp;第".$i."页&nbsp;&nbsp;".$Htmllinks."<font color=green>生成完毕~!</font></a><br/>";
                  $n++;
                  ob_flush();flush();
	...
	public function show_save()
	{
            ...
            if($datacount==0){
				 $sqlstr="select count(*) as count from ".MS_SqlPrefix."news where yid=0 and hid=0 ".$str.$limit;
                 $rows=$this->db->query($sqlstr)->result_array();
				 $datacount=$rows[0]['count'];
    	         $pagejs = ceil($datacount/Html_PageNum);
			}
            ...
            $sql_string="select * from ".MS_SqlPrefix."news where yid=0 and hid=0 ".$str." order by id desc";
            $sql_string.=' limit '. $pagesize*($page-1) .','. $pagesize;
			$query = $this->db->query($sql_string);
            ...
            foreach ($query->result_array() as $row) {
				  ...
				  //装载模板并输出
	        	  $Mark_Text=$this->MsdjTpl->plub_show('news',$row,$arr,TRUE,$skins,$row['name'],$row['name']);
				  //评论
				  ...
				  $Mark_Text=str_replace("[news:classname]",$this->MsdjDB->getzd('news_list','name',$row['cid']),$Mark_Text);
				  //获取上下篇
            	  preg_match_all('/[news:slink]/',$Mark_Text,$arr);
		    	  if(!empty($arr[0]) && !empty($arr[0][0])){
                   	  $rowd=$this->db->query("Select id,cid,name from ".MS_SqlPrefix."news where yid=0 and hid=0 and id<".$id." order by id desc limit 1")->row();
				  	  if($rowd){
		    	  ...
		    	  if(!empty($arr[0]) && !empty($arr[0][0])){
                  	  $rowd=$this->db->query("Select id,cid,name from ".MS_SqlPrefix."news where yid=0 and hid=0 and id>".$id." order by id asc limit 1")->row();
				  	  if($rowd){
