...
class Topic extends msvod_Controller {
	...
	public function index()
	{
            ...
            $sql_string = "SELECT id,name,pic,hits,tid,yid,addtime FROM ".MS_SqlPrefix."news_topic where 1=1";
			if($yid==1){
	...
	public function init()
	{
            ...
            $this->MsdjDB->get_update('news_topic',$id,$edit);
            die($str);
	}
	...
	public function edit()
	{
			...
			if($id==0){
                ...
                $row=$this->db->query("SELECT * FROM ".MS_SqlPrefix."news_topic where id=".$id."")->row(); 
	...
	public function save()
	{
			...
			if($id==0){ //新增
                 $this->MsdjDB->get_insert('news_topic',$data);
			}else{
                 $this->MsdjDB->get_update('news_topic',$id,$data);
			}
	...
	public function del()
	{
			...
			$result=$this->db->query("SELECT pic,toppic FROM ".MS_SqlPrefix."news_topic where id in(".$idss.")")->result();
			$this->load->library('msup');
			foreach ($result as $row) {
                  if(!empty($row->pic)){
					    $this->msup->del($row->pic,'newstopic'); //删除图片
				  }
                  if(!empty($row->toppic)){
					    $this->msup->del($row->toppic,'newstopic'); //删除顶部图
				  }
			}
			$this->MsdjDB->get_del('news_topic',$ids);
