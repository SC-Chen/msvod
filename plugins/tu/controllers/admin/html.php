...
class Html extends msvod_Controller {
	...
	public function topic_save()
	{
            ...
            if($datacount==0){
				 ...
				 $sqlstr="select count(*) as count from ".MS_SqlPrefix."tu_topic where yid=0";
            ...
            for($i=$start;$i<=$pagejs;$i++){
				  ...
				  $Mark_Text=$this->MsdjTpl->plub_list(array(),1,'lists',$page,'',true,'topic.html','topic','','图片专题');
			...
			$str="&nbsp;&nbsp;<b><font color=#0000ff>所有专题列表全部生成完毕!</font>&nbsp;>>>>&nbsp;&nbsp;<a href='".site_url('tu/admin/html/topic')."'>如果您的 浏览器没有跳转，请点击继续...</a></b>";
	...
	public function topicshow_save()
	{
            ...
            if($datacount==0){
				 $sqlstr="select count(*) as count from ".MS_SqlPrefix."tu_topic where yid=0 ".$str;
                 $rows=$this->db->query($sqlstr)->result_array();
				 $datacount=$rows[0]['count'];
	        ...
	        if($datacount==0 || $page>$pagejs){
                   admin_msg('所有专题内容页全部生成完毕~!',site_url('tu/admin/html/topic'));
			}
            ...
            foreach ($query->result_array() as $row) {
                  ...
                  //生成
			      write_file(FCPATH.$Htmllink,$Mark_Text);
				  echo "&nbsp;<font style=font-size:10pt;>生成图片专题:<font color=red>".$row['name']."</font>成功:<a href=".$Htmllinks." target=_blank>".$Htmllinks."</a></font><br/>";
                  ob_flush();flush();
	...
	public function type_save()
	{
            if($this->huri['lists']['check']==0){
				admin_msg('图片分类未开启生成~!','javascript:history.back();','no');
			}
			...
			//没有选择分类
			if(empty($cid)){
                    admin_msg('请选择要生成的分类~!',site_url('tu/admin/html/type'));
			}
            ...
            //全部生成完毕
	        if($nums>=$len){
                    admin_msg('所有分类全部生成完毕~!',site_url('tu/admin/html/type'));
			}
            ...
            if($datacount==0){
				 ...
				 $sqlstr="select count(*) as count from ".MS_SqlPrefix."tu where yid=0 and hid=0 and cid IN (". getChild($id).")";
                 $rows=$this->db->query($sqlstr)->result_array();
				 $datacount=$rows[0]['count'];
    	         $pagejs = ceil($datacount/$pagesize);
			}
			...
			if(($numx+1)>=$len2){ //全部完成
                $url=site_url('tu/admin/html/type_save').$uri.'&nums='.($nums+1);
				$str="&nbsp;&nbsp;<b><font color=#0000ff>所有排序页面全部生成完毕!</font>&nbsp;>>>>&nbsp;&nbsp;<a href='".site_url('tu/admin/html/type_save').$uri.'&nums='.($nums+1)."'>如果您的 浏览器没有跳转，请点击继续...</a></b>";
			}else{ //当前排序方式完成
                $url=site_url('tu/admin/html/type_save').$uri.'&nums='.$nums.'&numx='.($numx+1);
				$str='&nbsp;&nbsp;<b>按<font color=#0000ff>'.$this->gettype($type).'排序</font>分类生成完毕!&nbsp;>>>>&nbsp;&nbsp;<a href="'.site_url('tu/admin/html/type_save').$uri.'&nums='.$nums.'&numx='.($numx+1).'">如果您的 浏览器没有跳转，请点击继续...</a></b>';
			}
			echo("</br>".$str."<script>setTimeout('updatenext();',".Html_StopTime."000);function updatenext(){location.href='".$url."';}</script>");
	}
	...
	public function show_save()
	{
            if($this->huri['show']['check']==0){
				admin_msg('图片内容页未开启生成~!','javascript:history.back();','no');
			}
            ...
            if($datacount==0){
				 $sqlstr="select count(*) as count from ".MS_SqlPrefix."tu where yid=0 and hid=0 ".$str.$limit;
                 $rows=$this->db->query($sqlstr)->result_array();
				 $datacount=$rows[0]['count'];
    	         $pagejs = ceil($datacount/Html_PageNum);
			}
            ...
            //全部生成完毕
	        if($page>$pagejs){
                   admin_msg('所有内容页全部生成完毕~!',site_url('tu/admin/html/show'));
			}
            ...
            echo '<LINK href="'.base_url().'packs/admin/css/style.css" type="text/css" rel="stylesheet"><br>';
			echo '&nbsp;&nbsp;<b>正在开始生成图片内容,分<font color=red>'.$pagejs.'</font>次生成，当前第<font color=red>'.$page.'</font>次</b><br/>';
            ...
            foreach ($query->result_array() as $row) {
                  ...
                  //生成
			      write_file(FCPATH.$Htmllink,$Mark_Text);
				  echo "&nbsp;<font style=font-size:10pt;>生成图片:<font color=red>".$row['name']."</font>成功:<a href=".$Htmllinks." target=_blank>".$Htmllinks."</a></font><br/>";
                  ob_flush();flush();
