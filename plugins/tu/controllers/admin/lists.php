...
class Lists extends msvod_Controller {
	...
	{
            $sql_string = "SELECT * FROM ".MS_SqlPrefix."tu_list where fid=0 order by xid asc";
	        $query = $this->db->query($sql_string); 
	        $data['tu_list'] = $query->result();
            $this->load->view('tu_list.html',$data);
	}
	...
	public function init()
	{
            ...
            if($ac=='zt'){ //显示
                  $edit['yid']=$sid;
				  $str=($sid==0)?'<a title="点击隐藏" href="javascript:get_cmd(\''.site_url('tu/admin/lists/init').'?sid=1\',\'zt\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/ok.gif" /></a>':'<a title="点击显示" href="javascript:get_cmd(\''.site_url('tu/admin/lists/init').'?sid=0\',\'zt\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/no.gif" /></a>';
			}
            $this->MsdjDB->get_update('tu_list',$id,$edit);
            die($str);
	}
	...
	public function plsave()
	{
			...
			foreach ($ids as $id) {
                    ...
                    $this->MsdjDB->get_update('tu_list',$id,$data);
			}
            admin_msg('恭喜您，操作成功~!',site_url('tu/admin/lists'),'ok');  //操作成功
	}
	...
	public function zhuan()
	{
			...
			$ids=implode(',', $ids);
            $this->db->query("update ".MS_SqlPrefix."tu set cid=".$cid." where cid in (".$ids.")");
            admin_msg('恭喜您，操作成功~!',site_url('tu/admin/lists'),'ok');  //操作成功
	}
	...
	public function edit()
	{
			...
			if($id==0){
                ...
                $row=$this->db->query("SELECT * FROM ".MS_SqlPrefix."tu_list where id=".$id."")->row(); 
	...
	public function save()
	{
			...
			if($id==0){ //新增
                 $this->MsdjDB->get_insert('tu_list',$data);
			}else{
                 $this->MsdjDB->get_update('tu_list',$id,$data);
			}
            admin_msg('恭喜您，操作成功~!',site_url('tu/admin/lists'),'ok');  //操作成功
	...
	public function del()
	{
			...
			$this->MsdjDB->get_del('tu_list',$ids,'fid');
			$this->MsdjDB->get_del('tu_list',$ids);
