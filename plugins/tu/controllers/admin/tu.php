...
class Tu extends msvod_Controller {
	...
	public function index()
	{
            ...
            $sql_string = "SELECT id,name,pic,hits,hid,reco,cid,uid,yid,addtime FROM ".MS_SqlPrefix."tu where 1=1";
			if($yid==1){
            ...
            $base_url = site_url('tu/admin/tu')."?yid=".$yid."&zd=".$zd."&key=".$key."&cid=".$cid."&sort=".$sort."&reco=".$reco;
	        $per_page = 15; 
	...
	public function init()
	{
            ...
            if($ac=='zt'){ //锁定
                  $edit['yid']=$sid;
				  if($sid==0) $this->dt($id);
				  $str=($sid==0)?'<a title="点击锁定" href="javascript:get_cmd(\''.site_url('tu/admin/tu/init').'?sid=1\',\'zt\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/ok.gif" /></a>':'<a title="点击解除锁定" href="javascript:get_cmd(\''.site_url('tu/admin/tu/init').'?sid=0\',\'zt\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/no.gif" /></a>';
			}elseif($ac=='tj'){  //推荐
                  $edit['reco']=$sid;
		          $str='<a title="点击取消推荐" href="javascript:get_cmd(\''.site_url('tu/admin/tu/init').'?sid=0\',\'tj\','.$id.');"><font color="#ff0033">×</font></a>';
                  for($i=1;$i<=$sid;$i++){
                      $str.='<a title="推荐：'.$i.'星" href="javascript:get_cmd(\''.site_url('tu/admin/tu/init').'?sid='.$i.'\',\'tj\','.$id.');">★</a>';
                  }
                  for($j=$sid+1;$j<=5;$j++){
                      $str.='<a title="推荐：'.$j.'星" href="javascript:get_cmd(\''.site_url('tu/admin/tu/init').'?sid='.$j.'\',\'tj\','.$id.');">☆</a>';
                  }
			}
            $this->MsdjDB->get_update('tu',$id,$edit);
            die($str);
	}
	...
	public function edit()
	{
			...
			if($id==0){
                ...
                $row=$this->db->query("SELECT * FROM ".MS_SqlPrefix."tu where id=".$id."")->row(); 
            ...
            $this->load->view('tu_edit.html',$data);
	}
	...
	public function save()
	{
            ...
            if(empty($name)||empty($data['cid'])){
                   admin_msg('抱歉，图片名称、分类不能为空~!','javascript:history.back();','no');
			}
			...
			if($id==0){ //新增
				 $data['addtime']=time();
                 $this->MsdjDB->get_insert('tu',$data);
			}else{
				 if($data['yid']==0) $this->dt($id);
                 if($addtime=='ok') $data['addtime']=time();
                 $this->MsdjDB->get_update('tu',$id,$data);
			}
            admin_msg('恭喜您，操作成功~!',site_url('tu/admin/tu'),'ok');  //操作成功
	}
	...
	public function del()
	{
			...
			//回收站
			if($ac=='hui'){
			     $result=$this->db->query("SELECT id,pic,pic2 FROM ".MS_SqlPrefix."tu where hid=1")->result();
			     $this->load->library('msup');
			     foreach ($result as $row) {
                     if(!empty($row->pic)){
					    $this->msup->del($row->pic,'tu'); //删除图片
				     }
                     if(!empty($row->pic2)){
					    $this->msup->del($row->pic2,'tu'); //删除幻灯图
				     }
					 $this->MsdjDB->get_del('tu',$row->id);
				 }
			...
			if($yid==3){
			     $result=$this->db->query("SELECT pic,pic2 FROM ".MS_SqlPrefix."tu where id in(".$idss.")")->result();
			     $this->load->library('msup');
			     foreach ($result as $row) {
                     if(!empty($row->pic)){
					    $this->msup->del($row->pic,'tu'); //删除图片
				 ...
				 $data['hid']=1;
			     $this->MsdjDB->get_update('tu',$ids,$data);
	...
	public function pl_save()
	{
            ...
            if($xid==1){  //按ID操作
				   if(empty($id)) admin_msg('请选择要操作的图片ID~!','javascript:history.back();','no');
                   foreach ($csid as $v) {
                          if($v=="cid"){
					          $this->db->query("update ".MS_SqlPrefix."tu set cid=".$cid." where id in (".$id.")");
                          }elseif($v=="yid"){
							  //增加金币、经验
							  if($yid==0){
								 if(is_array($id)){
					          ...
					          $this->db->query("update ".MS_SqlPrefix."tu set yid=".$yid." where id in (".$id.")");
                          }elseif($v=="tid"){
					          $this->db->query("update ".MS_SqlPrefix."tu set tid=".$tid." where id in (".$id.")");
                          }elseif($v=="reco"){
					          $this->db->query("update ".MS_SqlPrefix."tu set reco=".$reco." where id in (".$id.")");
                          }elseif($v=="cion"){
					          $this->db->query("update ".MS_SqlPrefix."tu set ciond=".$cion." where id in (".$id.")");
                          }elseif($v=="vip"){
					          $this->db->query("update ".MS_SqlPrefix."tu set vip=".$vip." where id in (".$id.")");
                          }elseif($v=="hits"){
					          $this->db->query("update ".MS_SqlPrefix."tu set hits=".$hits." where id in (".$id.")");
                          }elseif($v=="yhits"){
					          $this->db->query("update ".MS_SqlPrefix."tu set yhits=".$yhits." where id in (".$id.")");
                          }elseif($v=="zhits"){
					          $this->db->query("update ".MS_SqlPrefix."tu set zhits=".$zhits." where id in (".$id.")");
                          }elseif($v=="rhits"){
					          $this->db->query("update ".MS_SqlPrefix."tu set rhits=".$rhits." where id in (".$id.")");
                          }elseif($v=="user"){
					          $uid=intval(getzd('user','id',$user,'name'));
					          $this->db->query("update ".MS_SqlPrefix."tu set uid=".$uid." where id in (".$id.")");
				          }elseif($v=="hid"){
					          if($hid==2){
                                  $this->MsdjDB->get_del('tu',$id);
					          }else{
							      //删除金币、经验
							      if(is_array($id)){
					              ...
					              $this->db->query("update ".MS_SqlPrefix."tu set hid=".$hid." where id in (".$id.")");
					          }
				          }
				   }
			}else{ //按分类操作
				   if(empty($cids)) admin_msg('请选择要操作的图片分类~!','javascript:history.back();','no');
                   foreach ($csid as $v) {
                          if($v=="cid"){
					          $this->db->query("update ".MS_SqlPrefix."tu set cid=".$cid." where cid in (".$cids.")");
                          }elseif($v=="yid"){
					          $this->db->query("update ".MS_SqlPrefix."tu set yid=".$yid." where cid in (".$cids.")");
                          }elseif($v=="tid"){
					          $this->db->query("update ".MS_SqlPrefix."tu set tid=".$tid." where cid in (".$cids.")");
                          }elseif($v=="reco"){
					          $this->db->query("update ".MS_SqlPrefix."tu set reco=".$reco." where cid in (".$cids.")");
                          }elseif($v=="cion"){
					          $this->db->query("update ".MS_SqlPrefix."tu set cion=".$cion." where cid in (".$cids.")");
                          }elseif($v=="vip"){
					          $this->db->query("update ".MS_SqlPrefix."tu set vip=".$vip." where cid in (".$cids.")");
                          }elseif($v=="hits"){
					          $this->db->query("update ".MS_SqlPrefix."tu set hits=".$hits." where cid in (".$cids.")");
                          }elseif($v=="yhits"){
					          $this->db->query("update ".MS_SqlPrefix."tu set yhits=".$yhits." where cid in (".$cids.")");
                          }elseif($v=="zhits"){
					          $this->db->query("update ".MS_SqlPrefix."tu set zhits=".$zhits." where cid in (".$cids.")");
                          }elseif($v=="rhits"){
					          $this->db->query("update ".MS_SqlPrefix."tu set rhits=".$rhits." where cid in (".$cids.")");
                          }elseif($v=="user"){
					          $uid=intval(getzd('user','id',$user,'name'));
					          $this->db->query("update ".MS_SqlPrefix."tu set uid=".$uid." where cid in (".$cids.")");
				          }elseif($v=="hid"){
					          if($hid==2){
                                  $this->MsdjDB->get_del('tu',$cids);
					          }else{
					              $this->db->query("update ".MS_SqlPrefix."tu set hid=".$hid." where cid in (".$cids.")");
					          }
				          }
				   }
			}
	...
	{
			$dt=$this->db->query("SELECT id,name,yid FROM ".MS_SqlPrefix."dt where link='".linkurl('show','id',$id,1,'tu')."'")->row();
			if($dt){
                  $uid=getzd('tu','uid',$id);
				  if($sid>0){ //删除回收站
				      ...
				      if($addhits<User_Nums_Add){
                         $this->db->query("update ".MS_SqlPrefix."user set cion=cion+".User_Cion_Add.",jinyan=jinyan+".User_Jinyan_Add.",addhits=addhits+1 where id=".$uid."");
					     $str.=L('plub_99');
				      }
