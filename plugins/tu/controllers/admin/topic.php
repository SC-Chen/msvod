...
class Topic extends msvod_Controller {
	...
	public function index()
	{
            ...
            $sql_string = "SELECT id,name,pic,hits,tid,yid,addtime FROM ".MS_SqlPrefix."tu_topic where 1=1";
			if($yid==1){
            ...
            $base_url = site_url('tu/admin/topic')."?yid=".$yid."&key=".$key."&sort=".$sort."&tid=".$tid;
	        $per_page = 15; 
	...
	public function init()
	{
            ...
            if($ac=='zt'){ //锁定
                  $edit['yid']=$sid;
				  $str=($sid==0)?'<a title="点击锁定" href="javascript:get_cmd(\''.site_url('tu/admin/topic/init').'?sid=1\',\'zt\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/ok.gif" /></a>':'<a title="点击解除锁定" href="javascript:get_cmd(\''.site_url('tu/admin/topic/init').'?sid=0\',\'zt\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/no.gif" /></a>';
			}elseif($ac=='tj'){  //推荐
                  $edit['tid']=$sid;
		          $str=($sid==1)?'<a title="点击取消推荐" href="javascript:get_cmd(\''.site_url('tu/admin/topic/init').'?sid=0\',\'tj\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/ok.gif" /></a>':'<a title="点击推荐" href="javascript:get_cmd(\''.site_url('tu/admin/topic/init').'?sid=1\',\'tj\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/no.gif" /></a>';
			}
            $this->MsdjDB->get_update('tu_topic',$id,$edit);
            die($str);
	}
	...
	public function edit()
	{
			...
			if($id==0){
                ...
                $row=$this->db->query("SELECT * FROM ".MS_SqlPrefix."tu_topic where id=".$id."")->row(); 
	...
	public function save()
	{
			...
			if($id==0){ //新增
                 $this->MsdjDB->get_insert('tu_topic',$data);
			}else{
                 $this->MsdjDB->get_update('tu_topic',$id,$data);
			}
            admin_msg('恭喜您，操作成功~!',site_url('tu/admin/topic'),'ok');  //操作成功
	}
	...
	public function del()
	{
			...
			$result=$this->db->query("SELECT pic,toppic FROM ".MS_SqlPrefix."tu_topic where id in(".$idss.")")->result();
			$this->load->library('msup');
			foreach ($result as $row) {
                  if(!empty($row->pic)){
					    $this->msup->del($row->pic,'tutopic'); //删除图片
				  }
                  if(!empty($row->toppic)){
					    $this->msup->del($row->toppic,'tutopic'); //删除顶部图
				  }
			}
			$this->MsdjDB->get_del('tu_topic',$ids);
