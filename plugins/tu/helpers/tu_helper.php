...
function getChild($CID,$type='tu_list',$zd='fid'){
	 $ci = &get_instance();
	 if (!isset($ci->db)){
        $ci->load->database();
	 }
     if(!empty($CID)){
            ...
            for($i=0;$i<count($ClassArr);$i++){
				$sql="select id from ".MS_SqlPrefix.$type." where ".$zd."='$ClassArr[$i]'";//sql语句的组织返回
...
//获取数据总数
function getcount($table='tu',$sid=0){
	 $ci = &get_instance();
	 if (!isset($ci->db)){
        $ci->load->database();
	 }
	 $sql="select count(*) as count from ".MS_SqlPrefix.$table." where yid=0 and hid=0";
     if($sid>0){
...
function SearchLink($Search_Key,$fid='tags'){
	 $Search_link='http://'.Web_Url.Web_Path.'index.php/tu/'; //板块地址
	 ...
	 //开启二级域名
	 $Ym_Mode=config('Ym_Mode','tu'); //二级域名状态
	 $Ym_Url=config('Ym_Url','tu');   //二级域名地址
	 $Web_Mode=config('Web_Mode','tu');   //版块运行模式
	 if($Ym_Mode==1){
		 if($Web_Mode==2){
	          $Search_List  = str_replace(Web_Url.Web_Path."index.php/tu/",$Ym_Url.Web_Path,$Search_List);
		 }else{
	          $Search_List  = str_replace(Web_Url.Web_Path."index.php/tu/",$Ym_Url.Web_Path."index.php/",$Search_List);
		 }
	 }
	 if($Web_Mode==2){
