...
class Denglu {
	...
	public function login($ac, $log_state = '') {
		if (MS_Appmode == 1) {
			//官方登录
			$log_url = "http://denglu.msvod.cc/denglu?ac=" . $ac . "&appid=" . MS_Appid . "&redirect_uri=" . $this->redirect_uri . "&state=" . $log_state . "&getdate=" . time();
	...
	public function callback($ac, $log_state = '') {
		if (MS_Appmode == 1) {
			//官方登录
			return $this->msvod_callback($ac, $log_state);
		} else {
			//独立
			$mode = $ac . '_callback';
	...
	public function qq_login($log_state = '') {
		...
		$login_url = 'https://graph.qq.com/oauth2.0/authorize?response_type=code&client_id=' . MS_Qqid . '&redirect_uri=' . $this->redirect_uri . '&state=' . $log_state . '&scope=' . $scope;
	...
	public function baidu_login($log_state = '') {
		...
		$login_url = 'https://openapi.baidu.com/oauth/2.0/authorize?response_type=code&client_id=' . MS_Bdid . '&redirect_uri=' . $this->redirect_uri . '&scope=netdisk&display=';
		header("Location:$login_url");
	}
	...
	public function renren_login($log_state = '') {
		...
		$login_url = "http://graph.renren.com/oauth/authorize?client_id=" . MS_Rrid . "&redirect_uri=" . $this->redirect_uri . "&response_type=code&state=" . $log_state . "&display=&x_renew=";
		header("Location:$login_url");
	}
	...
	public function kaixin_login($log_state = '') {
		...
		$login_url = "http://api.kaixin001.com/oauth2/authorize?response_type=code&client_id=" . MS_Kxid . "&redirect_uri=" . $this->redirect_uri . "&state=" . $log_state . "&scope=basic&display=popup";
		header("Location:$login_url");
	}
	...
	public function douban_login($log_state = '') {
		...
		$login_url = "https://www.douban.com/service/auth2/auth?response_type=code&client_id=" . MS_Dbid . "&redirect_uri=" . $this->redirect_uri . "&state=" . $log_state . "&scope=&display=" . $scope;
		header("Location:$login_url");
	}

	//官方登录返回
	public function msvod_callback($ac, $log_state = '') {
		...
		//查看数据库是否存在
		$row = $this->ci->db->query("SELECT id,uid,nickname,avatar FROM " . MS_SqlPrefix . "useroauth where csid=" . intval($uid) . "")->row();
		if ($row) {
			...
			$add['access_token'] = '';
			$add['refresh_token'] = '';
			$ids = $this->ci->MsdjDB->get_insert('useroauth', $add);
	...
	public function qq_callback($log_state = '') {
		...
		. "client_id=" . MS_Qqid . "&redirect_uri=" . urlencode($this->redirect_uri)
			. "&client_secret=" . MS_Qqkey . "&code=" . $code;
	...
	function weibo_callback($log_state = '') {
		...
		if ($access_token && $uid) {
			//获取用户信息
			$url = "https://api.weibo.com/2/users/show.json?source=" . MS_Wbkey . "&access_token=" . $access_token . "&uid=" . $uid;
	...
	function baidu_callback($log_state = '') {
		...
		if ($code) {
			...
			. "client_id=" . MS_Bdid . "&redirect_uri=" . urlencode($this->redirect_uri)
				. "&client_secret=" . MS_Bdkey . "&code=" . $code;
			$response = $this->get_url_contents($token_url);
	...
	function renren_callback($log_state = '') {
		...
		if ($log_state == $state) {
			...
			. "client_id=" . MS_Rrid . "&redirect_uri=" . urlencode($this->redirect_uri)
				. "&client_secret=" . MS_Rrkey . "&code=" . $code;
	...
	function kaixin_callback($log_state = '') {
		...
		if ($log_state == $state) {
			...
			$post = "grant_type=authorization_code&"
			. "client_id=" . MS_Kxid . "&redirect_uri=" . urlencode($this->redirect_uri)
				. "&client_secret=" . MS_Kxkey . "&code=" . $code;
			$response = $this->get_url_contents($token_url, $post, 'post');
	...
	function douban_callback($log_state = '') {
		...
		if ($log_state == $state) {
			...
			$post = "grant_type=authorization_code&"
			. "client_id=" . MS_Dbid . "&redirect_uri=" . urlencode($this->redirect_uri)
				. "&client_secret=" . MS_Dbkey . "&code=" . $code;
			$response = $this->get_url_contents($token_url, $post, 'post');
	...
	function denglu_str($ac, $oid, $user = array()) {
		...
		$row = $this->ci->db->query("SELECT id,uid,nickname,avatar FROM " . MS_SqlPrefix . "useroauth where oid='" . $oid . "' and cid=" . $cid . "")->row();
		if ($row) {
			$edit['access_token'] = $user['access_token'];
			$edit['refresh_token'] = $user['refresh_token'];
			$edit['expire_at'] = (int) $user['expire_in'];
			$this->ci->MsdjDB->get_update('useroauth', $row->id, $edit);
			$_SESSION['denglu__id'] = $row->id;
			$_SESSION['denglu__name'] = $row->nickname;
			$_SESSION['denglu__logo'] = $row->avatar;
			return $row->uid;
