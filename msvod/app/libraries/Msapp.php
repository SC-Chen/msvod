...
class Msapp {
	...
	function url($mx='plub',$data=''){
		$url = MS_YPTURL.$mx.'?time='.time().'&param='.ms_base64_encode(arraystring(array(
			'site' => Web_Url,
	...
	function keys($data,$mx='plub'){
		$url = MS_YPTURL.$mx.'/key?param='.ms_base64_encode(arraystring(array(
			'site' => Web_Url,
			'url'  => 'http://'.Web_Url.Web_Path,
			'data' => $data,
			'admin' => SELF,
			'encry' => MS_Encryption_Key,
		)));
        return htmlall($url);
    }
