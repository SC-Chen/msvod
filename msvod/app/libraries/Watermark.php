...
class Watermark {
  		...
  		* **/
  		...
  		public function imagewatermark($filename){
          		...
          		$watertype=MS_WaterMode; //水印类型(1为文字,2为图片) 
          		$waterposition=MS_WaterLocation; //文字水印位置(1为左下角,2为右下角,3为左上角,4为右上角,5为居中);  
          		$waterpositions=MS_WaterLocations; //图片水印位置(1为左下角,2为右下角,3为左上角,4为右上角,5为居中);  
          		$waterstring=MS_WaterFont; //水印字符串  
          		$waterstringw=130; //水印字符串的高度
          		$waterstringh=20; //水印字符串的高度
          		$waterstringpadding = 1; //水印字符串的居上右下左
          		$waterimg=FCPATH.MS_WaterLogo; //水印图片  
          		$imgquality = MS_WaterLogotm; //图片质量0-100，值最大图片质量愈好，图片的大小也越大 *推荐90-100 太小图片会出现模糊现象
            	...
            	if(MS_WaterFontColor!="") { 
              			$R = hexdec(substr(MS_WaterFontColor,1,2)); 
              			$G = hexdec(substr(MS_WaterFontColor,3,2)); 
              			$B = hexdec(substr(MS_WaterFontColor,5)); 
				} else { 
