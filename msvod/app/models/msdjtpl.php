...
class MsdjTpl extends CI_Model
{
//构造方法初始化
...
$Mark_Text=str_replace("{msvod:title}",$seo_title,$Mark_Text);
$Mark_Text=str_replace("{msvod:keywords}",$seo_keywords,$Mark_Text);
$Mark_Text=str_replace("{msvod:description}",$seo_description,$Mark_Text);
...
$Mark_Text=str_replace("{msvod:keywords}",$seo_keywords,$Mark_Text);
$Mark_Text=str_replace("{msvod:description}",$seo_description,$Mark_Text);
$Mark_Text=str_replace("{msvod:fid}",$fid,$Mark_Text); //当前使用的fid
$Mark_Text=str_replace("{msvod:ids}",$id,$Mark_Text); //当前使用的id

//预先除了分页
$pagenum=getpagenum($Mark_Text);
preg_match_all('/{msvod:([\S]+)\s+(.*?pagesize=\"([\S]+)\".*?)}([\s\S]+?){\/msvod:\1}/',$Mark_Text,$page_arr);//判断是否有分页标识
if(!empty($page_arr) && !empty($page_arr[2])){

$fields=$page_arr[1][0]; //前缀名
//组装SQL数据
$sort=$this->get_sort($fid);
$sqlstr=$this->skins->msvod_sql($page_arr[1][0],$page_arr[2][0],$page_arr[0][0],$page_arr[3][0],$sort,$ids);
preg_match('/select\s*(.+)from/i', strtolower($sqlstr),$sqlarr);
if(!empty($sqlarr[1])){
$sqlstr2=str_replace($sqlarr[1],' count(*) as counta ',strtolower($sqlstr));
$msvodtj=$this->db->query($sqlstr2)->result_array();
$nums=$msvodtj[0]['counta']; //总数量
}else{
$nums=$this->db->query($sqlstr)->num_rows(); //总数量
}
$plist=preg_match('/{msvod:pagelist}/i', $Mark_Text);
...
$Mark_Text=$this->skins->msvod_skins($field,$Mark_Text,$Mark_Text,$row);//解析当前数据标签
$Mark_Text=$this->skins->template_parse($Mark_Text,true);
if ($return == FALSE){
echo $Mark_Text;
$this->cache->end(); //写入缓存
...
$fields=$page_arr[1][0]; //前缀名
//组装SQL数据
$sqlstr=$this->skins->msvod_sql($page_arr[1][0],$page_arr[2][0],$page_arr[0][0],$page_arr[3][0],'id',$ids,0,$sql);
preg_match('/select\s*(.+)from/i', strtolower($sqlstr),$sqlarr);
if(!empty($sqlarr[1])){
$sqlstr2=str_replace($sqlarr[1],' count(*) as counta ',strtolower($sqlstr));
$msvodtj=$this->db->query($sqlstr2)->result_array();
...
unset($page_arr);
$Mark_Text=$this->skins->msvod_common($Mark_Text,$row['skins']);
$Mark_Text=$this->skins->csskins($Mark_Text,$ids);
...
//组装SQL数据
$sqlstr="select {field} from `".MS_SqlPrefix.$dir."` where 1=1";
$sql=$fid=$key="";
foreach ($so as $k=>$v) {
...
$Mark_Text=str_replace("{msvod:".$k."}",$v,$Mark_Text);
$Mark_Text=str_replace("{msvod:key}",$v,$Mark_Text);
$fid.="&".$k."=".urlencode($v);
}
...
$Mark_Text=str_replace("{msvod:zm}",$zm,$Mark_Text);
...
$sqlstr=$this->skins->msvod_sql($page_arr[1][0],$page_arr[2][0],$page_arr[0][0],$page_arr[3][0],'',0,0,$sqlstr);
preg_match('/select\s*(.+)from/i', strtolower($sqlstr),$sqlarr);
if(!empty($sqlarr[1])){
$sqlstr2=str_replace($sqlarr[1],' count(*) as counta ',strtolower($sqlstr));
$msvodtj=$this->db->query($sqlstr2)->result_array();
...
unset($page_arr);
$Mark_Text=str_replace("{msvod:cid}",'',$Mark_Text);
$Mark_Text=str_replace("{msvod:zhuyan}",'',$Mark_Text);
$Mark_Text=str_replace("{msvod:daoyan}",'',$Mark_Text);
$Mark_Text=str_replace("{msvod:yuyan}",'',$Mark_Text);
$Mark_Text=str_replace("{msvod:diqu}",'',$Mark_Text);
$Mark_Text=str_replace("{msvod:year}",'',$Mark_Text);
$Mark_Text=str_replace("{msvod:tags}",'',$Mark_Text);
$Mark_Text=str_replace("{msvod:name}",'',$Mark_Text);
$Mark_Text=str_replace("{msvod:type}",'',$Mark_Text);
$Mark_Text=str_replace("{msvod:key}",'',$Mark_Text);
$Mark_Text=str_replace("{msvod:zm}",'',$Mark_Text);
$Mark_Text=$this->skins->msvod_common($Mark_Text);
...
$Mark_Text=str_replace("{msvod:keywords}",$seo_keywords,$Mark_Text);
$Mark_Text=str_replace("{msvod:description}",$seo_description,$Mark_Text);
$Mark_Text=str_replace("{msvod:id}",$id,$Mark_Text); //当前使用的id
$Mark_Text=$this->skins->msvod_common($Mark_Text);
$Mark_Text=$this->skins->csskins($Mark_Text,$ids);
$Mark_Text=$this->skins->msvod_skins($field,$Mark_Text,$Mark_Text,$row);//解析当前数据标签
$Mark_Text=$this->skins->template_parse($Mark_Text,true);
...
$wap=(!defined('MOBILE'))?1:2;
$cache_id ="msvod_gbook_".$wap;
if(!($this->cache->start($cache_id))){
$template=$this->load->view('gbook.html','',true);
$gbook="<div id='msvod_gbook'><img src='".Web_Path."packs/images/load.gif'>&nbsp;&nbsp;加载留言内容,请稍等......</div>\r\n<script type='text/javascript'>msvod_gbook(1,0,0);</script>";
$Mark_Text=str_replace("{msvod:gbook}",$gbook,$template);
$Mark_Text=$this->skins->template_parse($Mark_Text,true);
echo $Mark_Text;
$this->cache->end(); //写入缓存
}
...
return "<div id='msvod_gbook'>网站已经关闭了在线留言~!</div>";
}
$data_content='';
//装载模板
...
$field=$page_arr[1][0]; //前缀名
//组装SQL数据
$sqlstr=$this->skins->msvod_sql($page_arr[1][0],$page_arr[2][0],$page_arr[0][0],$page_arr[3][0],'id',0);
preg_match('/select\s*(.+)from/i', strtolower($sqlstr),$sqlarr);
if(!empty($sqlarr[1])){
$sqlstr2=str_replace($sqlarr[1],' count(*) as counta ',strtolower($sqlstr));
$msvodtj=$this->db->query($sqlstr2)->result_array();
...
$Arr=spanajaxpage($sqlstr,$nums,$page_arr[3][0],$pagenum,'msvod_gbook',$page);
if($nums==0){
$data_content.="";
}else{
$sorti=1;
...
return "<div id='msvod_pl' style='text-align:center;'><b>网站已经关闭了评论~!</b></div>";
}
if($did==0){ //参数错误
return "<div id='msvod_pl' style='text-align:center;'><b>参数错误，数据ID为空~!</b></div>";
}
$data_content='';
//装载模板
if($dir=='blog'){
...
//组装SQL数据
$arr['did']=$did;
$sql="SELECT {field} FROM ".MS_SqlPrefix."pl where dir='".$dir."'";
$sqlstr=$this->skins->msvod_sql($page_arr[1][0],$page_arr[2][0],$page_arr[0][0],$page_arr[3][0],'id',$arr,$cid,$sql);
preg_match('/select\s*(.+)from/i', strtolower($sqlstr),$sqlarr);
if(!empty($sqlarr[1])){
$sqlstr2=str_replace($sqlarr[1],' count(*) as counta ',strtolower($sqlstr));
$msvodtj=$this->db->query($sqlstr2)->result_array();
...
$Arr=spanajaxpage($sqlstr,$nums,$page_arr[3][0],$pagenum,'msvod_pl',$page);
if($nums==0){
$data_content.="";
}else{
$sorti=1;
...
$plfaces="";
for($i=1;$i<=56;$i++){
$plfaces.="<img style='cursor:pointer;' src=\"".Web_Path."packs/images/faces/e".$i.".gif\" onclick=\"$('#msvod_pl_content').val($('#msvod_pl_content').val()+'[em:".$i."]');$('#msvod_faces').hide();\" />";
}
