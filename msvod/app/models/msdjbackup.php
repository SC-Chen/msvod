...
class MsdjBackup extends CI_Model {
			...
			if (strpos($table, MS_SqlPrefix) !== FALSE) {
				...
				foreach ($query->result_array() as $row) {
					...
					if (strlen($output) > ($size * 1024)) {
						$bkfile = "./attachment/backup/msvod_v4_" . date('Ymd') . "/datas_" . substr(md5(time() . mt_rand(1000, 5000)), 0, 16) . ".sql"; //名称
						//写文件
