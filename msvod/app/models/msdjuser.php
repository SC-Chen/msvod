...
if(time()>$time){
$times=time()-1800;
$this->db->query("update ".MS_SqlPrefix."user set jinyan=jinyan+".User_Jinyan_Zx.",cion=cion+".User_Cion_Zx." where zx=1");
$this->db->query("update ".MS_SqlPrefix."user set zx=0 where zx=1 and logms<".$times."");
write_file(FCPATH."cache/msvod_time/time.txt",time()+1800);
}
}
...
$id     = isset($_SESSION['msvod__id'])?intval($_SESSION['msvod__id']):0;
$logstr = isset($_SESSION['msvod__login'])?$_SESSION['msvod__login']:'';
}
...
$row=$this->db->query("SELECT id,name,pass,logo,level,jinyan FROM ".MS_SqlPrefix."user where id='$id'")->row();
if($row){
