...
class Uc extends msvod_Controller
{
  ...
  private function deleteuser($get, $post)
  {
      ...
      //删除用户接口
	  if($uid>0){
	     $row=$this->db->query("SELECT id FROM ".MS_SqlPrefix."user where uid=".$uid."")->row();
	     if($row){
		    $id=$row->id;
  ...
  private function synlogin($get, $post)
  {
      ...
      {
		    $row=$this->db->query("SELECT code,pass,sid,yid,id,name,lognum,cion,vip,logtime,viptime FROM ".MS_SqlPrefix."user where uid=".$uid."")->row();
		    if($row){
                     if($row->sid==0 && $row->yid==0){
						 if(!isset($_SESSION['msvod__id']) || $_SESSION['msvod__id']!=$row->id){
  ...
  private function updatepw($get, $post)
  {
	  ...
	  $row=$this->db->query("SELECT code,pass,id,name FROM ".MS_SqlPrefix."user where name='".$username."'")->row();
	  if($row){
           $edit['pass']=md5(md5(trim($get['password'])).$row->code);
           //修改
           $this->MsdjDB->get_update('user',$row->id,$edit);
	  }
      return self::API_RETURN_SUCCEED;
  }
  ...
  private function updatebadwords($get, $post)
  {
      ...
      $cachefile = MSVODPATH.'uc_client/data/cache/badwords.php';
      @unlink($cachefile);
      return self::API_RETURN_SUCCEED;
  }
  ...
  private function updatehosts($get, $post)
  {
      ...
      $cachefile = MSVODPATH.'uc_client/data/cache/hosts.php';
      @unlink($cachefile);
      return self::API_RETURN_SUCCEED;
  }
  ...
  private function updateapps($get, $post)
  {
      ...
      $cachefile = MSVODPATH.'uc_client/data/cache/apps.php';
      @unlink($cachefile);
      return self::API_RETURN_SUCCEED;
  }
  ...
  private function updateclient($get, $post)
  {
      ...
      $cachefile = MSVODPATH.'uc_client/data/cache/settings.php';
      @unlink($cachefile);
      return self::API_RETURN_SUCCEED;
  }
  ...
  public static function serialize($arr, $htmlOn = 0)
  {
      ...
      {
          require MSVODPATH.'uc_client/lib/xml.class.php';
      }
