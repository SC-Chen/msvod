...
class Ulog extends msvod_Controller {
	...
	public function log()
	{
		  ...
		  $Mark_Text=str_replace("{msvod:logadd}","msvod_logadd();",$Mark_Text);
		  $Mark_Text=str_replace("{msvod:logout}","msvod_logout();",$Mark_Text);
		  if(defined('HOMEPATH')){
		       $Mark_Text=$this->skins->msvod_common($Mark_Text,$skins);
		  }
		  if($login){
		        $row=$this->MsdjDB->get_row_arr('user','*',$_SESSION['msvod__id']);
				if(empty($row['nichen'])) $row['nichen']=$row['name'];
				$Mark_Text=$this->skins->msvod_skins('user',$Mark_Text,$Mark_Text,$row);
				$ucid=$row['uid'];
		  }
	...
	public function login()
	{
		  ...
		  $userpass=get_bm($userpass);
            ...
            if(empty($username)){
                ...
                //可以用会员名、邮箱来进行登入
                $sqlu="SELECT code,email,pass,sid,yid,id,name,lognum,cion,vip,logtime,viptime,zid,zutime FROM ".MS_SqlPrefix."user where name='".$username."' or email='".$username."'";
