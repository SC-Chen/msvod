...
class Pl extends msvod_Controller {
	...
	public function index($dir='blog',$did=0,$cid=0,$page=1)
	{
            ...
            $Mark_Text=$this->MsdjTpl->pl($dir,intval($did),intval($cid),intval($page));
	...
	public function add()
	{
			...
			if(Pl_Modes==3){
                 ...
                 $error='10003';
			}elseif(Pl_Youke==0 && empty($_SESSION['msvod__id'])){//关闭游客评论
                 $error='10004';
			}else{

                $add['uid']=isset($_SESSION['msvod__id'])?intval($_SESSION['msvod__id']):0;
                $add['user']=isset($_SESSION['msvod__name'])?$_SESSION['msvod__name']:'游客';
			    $add['cid']=intval($this->input->get_post('cid'));
			    $add['fid']=intval($this->input->get_post('fid'));
			    $add['ip']=getip();
			    $add['addtime']=time();

                $ids=$this->MsdjDB->get_insert('pl',$add);
			    if(intval($ids)==0){
                    $error='10005'; //失败
				}else{
                    //摧毁token
					...
					if($add['dir']!='singer'){
					    ...
					    if($add['dir']=='video'){
                             $link=linkurl('play','id',$add['did'],1,'video');
						}elseif($add['dir']!='blog'){
						...
						if($hf==0){
				             ...
				             $plneir=vsprintf(L('pl_03'),array($_SESSION['msvod__name'],$dirname,$link,$dataname));
						}else{
							 $pltitle=L('pl_02');
				             $plneir=vsprintf(L('pl_04'),array($_SESSION['msvod__name'],$link,$dataname));
							 $pluid=$uid;
					    }
						if($pluid>0){
				            $addm['uida']=$pluid;
	...
	public function del()
	{
			...
			if($id==0){
                 ...
                 $error='10002';
		            ...
		            $row=$this->MsdjDB->get_row('pl','uid',$id);
		            if(!$row){
                         $error='10001';
					}elseif($row->uid!=$_SESSION['msvod__id']){
                         $error='10003';
					}else{
                         //权限通过删除
						 $this->MsdjDB->get_del('pl',$id,'fid');
						 $this->MsdjDB->get_del('pl',$id);
                         $error='10004';
						 get_token('pl_token',2);
					}
			}
