...
class Sys extends msvod_Controller {
	...
	public function index()
	{
			...
			if($sid>0){
	             $sql_string = "SELECT * FROM ".MS_SqlPrefix."admin where sid='$sid' order by id desc";
			}else{
	             $sql_string = "SELECT * FROM ".MS_SqlPrefix."admin order by id desc";
			}
	...
	public function edit()
	{
            ...
            $row = $this->db->query("SELECT id,adminname,adminpass,sid FROM ".MS_SqlPrefix."admin where id='$id'")->row(); 
			if(!$row) admin_msg(L('plub_02'),site_url('admin/sys'),'no');
	...
	public function save()
	{
			...
			if($id==0){ //新增
				 ...
				 $row = $this->db->query("SELECT admincode FROM ".MS_SqlPrefix."admin where id='$id'")->row();
				 if(!empty($adminpass)){
                      $data['adminpass']=md5(md5($adminpass).$row->admincode);
				 }
                 $this->MsdjDB->get_update('admin',$id,$data);
			}
            admin_msg(L('plub_06'),site_url('admin/sys'),'ok');  //操作成功
	}

    //角色列表
	public function zu()
	{
	        $sql_string = "SELECT * FROM ".MS_SqlPrefix."adminzu order by id asc";
	        $query = $this->db->query($sql_string); 
	        $data['nums'] = $query->num_rows();
	...
	{
            $id=intval($this->uri->segment(4)); 
            $row = $this->db->query("SELECT id,name,sys FROM ".MS_SqlPrefix."adminzu where id='$id'")->row(); 
			if(!$row) admin_msg(L('plub_02'),site_url('admin/sys'),'no');
		    $data['name']=$row->name;
		    $data['sys']=$row->sys;
		    $data['id']=$row->id;
	...
	public function zu_save()
	{
			...
			if($id==0){ //新增
		         $data['sys']='';
		         $data['app']='';
                 $this->MsdjDB->get_insert('adminzu',$data);
			}else{
                 $this->MsdjDB->get_update('adminzu',$id,$data);
			}
            admin_msg(L('plub_06'),site_url('admin/sys/zu'),'ok');  //操作成功
	}
	...
	{
		    $adminpass=$this->input->post('adminpass', TRUE);
		    $row = $this->db->query("SELECT admincode FROM ".MS_SqlPrefix."admin where id='".$_SESSION['admin_id']."'")->row(); 
		    $data['adminname'] = $this->input->post('adminname', TRUE);
            ...
            $this->MsdjDB->get_update('admin',$_SESSION['admin_id'],$data);
            admin_msg(L('plub_09'),site_url('admin/login'),'ok');
	}
	...
	public function card()
	{
	        ...
	        $row = $this->db->query("SELECT card,adminname FROM ".MS_SqlPrefix."admin where id='".$id."'")->row(); 
			$data['picurl']='';
            if(!empty($row->card)){
	...
	public function card_add()
	{
		    $id=intval($this->uri->segment(4));
            if($id==0) admin_msg(L('plub_11'),'javascript:history.back();','no');
			$row = $this->db->query("SELECT adminname FROM ".MS_SqlPrefix."admin where id='".$id."'")->row(); 
            $this->load->library('card');
	        $card=$this->card->add($row->adminname);

			//修改口令卡
	        $updata['card'] = $card;
            $this->MsdjDB->get_update('admin',$id,$updata);
            admin_msg(L('plub_12'),site_url('admin/sys/card/'.$id),'ok','',5);
	}
	...
	public function card_del()
	{
            ...
            admin_msg($str,site_url('admin/sys/card/'.$id),'ok','',5);
	}
	...
	public function log()
	{
		    ...
		    //删除三个月以前的登录日志记录
			$times=time()-86400*90;
			$this->db->query("delete from ".MS_SqlPrefix."admin_log where logintime<".$times."");
            ...
            if($id>0){
	             $sql_string = "SELECT * FROM ".MS_SqlPrefix."admin_log where uid=".$id." order by ".$sort." ".$desc;
			}else{
	             $sql_string = "SELECT * FROM ".MS_SqlPrefix."admin_log order by ".$sort." ".$desc;
			}
	        $query = $this->db->query($sql_string); 
	        $total = $query->num_rows();

            $base_url = ($id==0)?site_url('admin/sys/log')."?sort=".$sort."&desc=".$desc:site_url('admin/sys/log/'.$id)."?sort=".$sort."&desc=".$desc;
	        $per_page = 15; 
	...
	public function zu_del()
	{
			...
			$this->MsdjDB->get_del('admin',$id,'sid'); //删除该角色下的所有成员
			$this->MsdjDB->get_del('adminzu',$id);
            admin_msg(L('plub_14'),'javascript:history.back();','ok');  //操作成功
	}
}
