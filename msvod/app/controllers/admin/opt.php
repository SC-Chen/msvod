...
class Opt extends msvod_Controller {
	...
	public
	function head() {
		$sqlstr = "select * from " . MS_SqlPrefix . "plugins order by id asc";
	...
	function main() {
		...
		if ( !file_exists( FCPATH . 'packs/install/plub_install.lock' ) ) {
			admin_msg( L( 'plub_opt_01' ), site_url( 'admin/opt/init' ), 'no', 'red' );
		}
		$data[ 'code' ] = ms_base64_encode( arraystring( array(
			'self' => Web_Path . SELF,
			'version' => MS_Version,
			'charset' => MS_Charset,
			'uptime' => MS_Uptime,
	...
	//清空数据库缓存
	public
	function del_dbcache() {
		deldir( './cache/' . MS_Cache_Dir . '/', 'no' );
		admin_msg( L( 'plub_opt_02' ), 'javascript:history.back();' );
	}

	//清空缩放图片缓存
	public
	function del_piccache() {
	...
	public
	function del_cache() {
		$sqlstr = "select name,dir from " . MS_SqlPrefix . "plugins order by id asc";
		$result = $this->db->query( $sqlstr );
	...
	function init() {
		...
		//判断安装
		if ( file_exists( FCPATH . 'packs/install/plub_install.lock' ) ) {
			admin_msg( L( 'plub_opt_04' ), site_url( 'admin/opt/main' ), 'no', 'red' );
		}
		// 搜索本地模块
