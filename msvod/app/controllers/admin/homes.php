...
class Homes extends msvod_Controller {
	...
	public function save() {
		...
		//写文件
		if (!write_file(MSVOD . 'lib/Ms_Home.php', $strs)) {
			admin_msg('./msvod/lib/Ms_Home.php ' . L('home_02'), 'javascript:history.back();', 'no');
		} else {
			admin_msg(L('home_03'), site_url('admin/homes'));
		}
	}
	...
	public function pay() {
		...
		$sql_string = "SELECT * FROM " . MS_SqlPrefix . "web_pay where 1=1";
		if (!empty($key)) {
			if ($zd == 'name') {
		...
		$base_url = site_url('admin/homes/pay') . "?zd=" . $zd . "&key=" . $key;
		$per_page = 15;
	...
	public function del() {
		...
		if (empty($id)) {
			admin_msg(L('home_04'), 'javascript:history.back();', 'no');
		}
		//数据不完成
		$this->MsdjDB->get_del('web_pay', $id);
		admin_msg(L('home_05'), site_url('admin/homes/pay'), 'ok'); //操作成功
	}
}
