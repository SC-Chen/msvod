...
class Login extends msvod_Controller {
	...
	function log_card() {
		...
		$log = $this->MsdjDB->get_row( 'admin', '*', $_SESSION[ 'card_code' ], 'card' );
		if ( $log ) {
			...
			$updata[ 'logip' ] = $ip;
			$updata[ 'logtime' ] = time();
			$this->MsdjDB->get_update( 'admin', $log->id, $updata );
			...
			$add[ 'logintime' ] = time();
			$add[ 'useragent' ] = $agent;
			$this->MsdjDB->get_insert( 'admin_log', $add );
