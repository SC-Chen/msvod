...
class User extends msvod_Controller {
...
$sql_string = "SELECT id,name,logo,email,vip,zid,sid,tid,rzid,regip,addtime,lognum,nichen,rmb,cion FROM ".MS_SqlPrefix."user where yid=0";
if($zid>0){
...
$base_url = site_url('admin/user')."?zid=".$zid."&zd=".$zd."&kstime=".$kstime."&jstime=".$jstime."&key=".$key."&sid=".$sid."&sort=".$sort."&desc=".$desc;
$per_page = 15; 
...
$sql_string = "SELECT id,name,email,regip,addtime FROM ".MS_SqlPrefix."user where yid>0  order by ".$sort." ".$desc;
$query = $this->db->query($sql_string); 
$total = $query->num_rows();
$base_url = site_url('admin/user/verify')."?sort=".$sort."&desc=".$desc;
$per_page = 15; 
$totalPages = ceil($total / $per_page); // 总页数
$data['nums'] = $total;
if($total<$per_page){
...
$sql_string = "SELECT * FROM ".MS_SqlPrefix."userzu  order by xid asc";
$query = $this->db->query($sql_string); 
$total = $query->num_rows();
$base_url = site_url('admin/user/zu');
$per_page = 15; 
$totalPages = ceil($total / $per_page); // 总页数
$data['nums'] = $total;
if($total<$per_page){
...
$sql_string = "SELECT * FROM ".MS_SqlPrefix."userlevel  order by stars asc";
$query = $this->db->query($sql_string); 
$total = $query->num_rows();
$base_url = site_url('admin/user/level');
$per_page = 15; 
$totalPages = ceil($total / $per_page); // 总页数
$data['nums'] = $total;
if($total<$per_page){
...
$this->MsdjDB->get_del('user',$id);
$emailneir=vsprintf(L('plub_02'),array($row->name));
}
$this->load->model('MsdjEmail');
$title=L('plub_03');
$res=$this->MsdjEmail->send($row->email,$title,$emailneir);  //发送通知邮件
...
$User_Cion_tichengy = intval($this->input->post('User_Cion_tichengy'));
$User_Cion_tichenge = intval($this->input->post('User_Cion_tichenge'));
$User_Cion_tichengs = intval($this->input->post('User_Cion_tichengs'));
$User_Sc = intval($this->input->post('User_Sc'));
...
global $_MS_Domain;
if(!empty($User_Ym)){
$_MS_Domain['user']=$User_Ym;
arr_file_edit($_MS_Domain);
}else{
if(arr_key_value($_MS_Domain,'user')){
unset($_MS_Domain['user']);
arr_file_edit($_MS_Domain);
}
}
...
include MSVOD.'lib/Ms_Ucenter.php';
...
if (!write_file(MSVOD.'lib/Ms_Ucenter.php', $strsuc)){
...
$strs.="define('User_RmbToCion',".$User_RmbToCion."); //默认金额比例  \r\n";
$strs.="define('User_Cion_Reg',".$User_Cion_Reg.");      //注册赠送金额  \r\n";
$strs.="define('User_Cion_Log',".$User_Cion_Log.");      //登入赠送金额  \r\n";
$strs.="define('User_Cion_Qd',".$User_Cion_Qd.");      //签到赠送金额\r\n";
$strs.="define('User_Cion_Logo',".$User_Cion_Logo.");      //上传头像赠送金额\r\n";
$strs.="define('User_Cion_Add',".$User_Cion_Add.");      //发表数据赠送金额\r\n";
$strs.="define('User_Cion_Zx',".$User_Cion_Zx.");      //在线1小时赠送金额\r\n";
$strs.="define('User_Cion_Del',".$User_Cion_Del.");      //数据删除扣除金额\r\n";
...
$strs.="define('User_Cion_Share',".$User_Cion_Share.");      //每次分享奖励金额\r\n";
...
$strs.="define('User_Cion_tichengy',".$User_Cion_tichengy.");      //代理充值提成\r\n";
$strs.="define('User_Cion_tichenge',".$User_Cion_tichenge.");      //代理充值提成\r\n";
$strs.="define('User_Cion_tichengs',".$User_Cion_tichengs.");      //代理充值提成\r\n";
$strs.="define('User_Sc',".$User_Sc.");        //会员上传视频方式\r\n";
$strs.="define('User_Skins','".$User_Skins."');      //会员默认模板路径";

//写文件
if (!write_file(MSVOD.'lib/Ms_User.php', $strs)){
admin_msg('./msvod/lib/Ms_User.php '.L('plub_09'),'javascript:history.back();','no');
}else{
admin_msg(L('plub_10'),site_url('admin/user/setting'));
}
}
...
$sid  = intval($this->input->get_post('sid'));
$edit['sid']=$sid;
$str=($sid==0)?'<a title="'.L('plub_11').'" href="javascript:get_cmd(\''.site_url('admin/user/init').'?sid=1\',\'zt\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/ok.gif" /></a>':'<a title="'.L('plub_12').'" href="javascript:get_cmd(\''.site_url('admin/user/init').'?sid=0\',\'zt\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/no.gif" /></a>';
}elseif($ac=='tj'){  //推荐
$tid  = intval($this->input->get_post('tid'));
$edit['tid']=$tid;
$str=($tid==1)?'<a title="'.L('plub_13').'" href="javascript:get_cmd(\''.site_url('admin/user/init').'?tid=0\',\'tj\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/ok.gif" /></a>':'<a title="'.L('plub_14').'" href="javascript:get_cmd(\''.site_url('admin/user/init').'?tid=1\',\'tj\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/no.gif" /></a>';
}else{  //认证
$rzid  = intval($this->input->get_post('rzid'));
$edit['rzid']=$rzid;
$str=($rzid==1)?'<a title="'.L('plub_15').'" href="javascript:get_cmd(\''.site_url('admin/user/init').'?rzid=0\',\'rz\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/ok.gif" /></a>':'<a title="'.L('plub_16').'" href="javascript:get_cmd(\''.site_url('admin/user/init').'?rzid=1\',\'rz\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/no.gif" /></a>';
}
$this->MsdjDB->get_update('user',$id,$edit);
die($str);
}
...
//删除三个月以前的登录日志记录
$times=time()-86400*90;
$this->db->query("delete from ".MS_SqlPrefix."user_log where logintime<".$times."");
...
if($id>0){
$sql_string = "SELECT * FROM ".MS_SqlPrefix."user_log where uid=".$id." order by ".$sort." ".$desc;
}else{
$sql_string = "SELECT * FROM ".MS_SqlPrefix."user_log order by ".$sort." ".$desc;
}
$query = $this->db->query($sql_string); 
$total = $query->num_rows();

$base_url = site_url('admin/user/log')."?id=".$id."&sort=".$sort."&desc=".$desc;
$per_page = 15; 
$totalPages = ceil($total / $per_page); // 总页数
$data['nums'] = $total;
if($total<$per_page){
...
$row=$this->db->query("SELECT * FROM ".MS_SqlPrefix."user where id=".$id."")->row(); 
...
if($id==0){ //新增
$data['addtime']=time();
$row=$this->db->query("SELECT id FROM ".MS_SqlPrefix."user where name='".$data['name']."'")->row(); 
if($row) admin_msg(L('plub_21'),'javascript:history.back();','no');  //用户名已经存在
$this->MsdjDB->get_insert('user',$data);
}else{
$this->MsdjDB->get_update('user',$id,$data);
}
//修改UC密码
if(!empty($pass)){
$data['pass']=md5(md5($pass).$data['code']);
...
if(User_Uc_Mode==1){
include MSVOD.'lib/Ms_Ucenter.php';
include MSVODPATH.'uc_client/client.php';
uc_user_edit($data['name'],'',$pass,'',1);
}
//--------------------------- Ucenter End ---------------------------
}
admin_msg(L('plub_06'),site_url('admin/user'),'ok');  //操作成功
}
...
if(is_array($id)){
foreach ($id as $id) {
$ucid = $this->MsdjDB->getzd('user','uid',$id);
if($ucid>0){
...
$this->MsdjDB->get_del('funco',$id,'uida'); //访客A
$this->MsdjDB->get_del('funco',$id,'uidb'); //访客B
$this->MsdjDB->get_del('fans',$id,'uida'); //粉丝A
$this->MsdjDB->get_del('fans',$id,'uidb'); //粉丝B
$this->MsdjDB->get_del('friend',$id,'uida');  //好友A
$this->MsdjDB->get_del('friend',$id,'uidb');   //好友B
$this->MsdjDB->get_del('gbook',$id,'uida'); //留言A
$this->MsdjDB->get_del('gbook',$id,'uidb'); //留言B
$this->MsdjDB->get_del('msg',$id,'uida'); //消息A
$this->MsdjDB->get_del('msg',$id,'uidb'); //消息B
$this->MsdjDB->get_del('pl',$id,'uid'); //评论
$this->MsdjDB->get_del('blog',$id,'uid'); //说说
$this->MsdjDB->get_del('dt',$id,'uid'); //动态
$this->MsdjDB->get_del('user_log',$id,'uid'); //登录日志
$this->MsdjDB->get_del('web_pay',$id,'uid'); //模板记录
$this->MsdjDB->get_del('pay',$id,'uid'); //充值记录
$this->MsdjDB->get_del('income',$id,'uid'); //收入记录
$this->MsdjDB->get_del('spend',$id,'uid'); //消费记录
$this->MsdjDB->get_del('share',$id,'uid'); //分享记录
$this->MsdjDB->get_del('session',$id,'uid'); //session会话
$this->MsdjDB->get_del('user',$id);
...
$row=$this->db->query("SELECT * FROM ".MS_SqlPrefix."userzu where id=".$id."")->row(); 
if(!$row) admin_msg(L('plub_17'),'javascript:history.back();','no');  //记录不存在
...
if($id==0){
$this->MsdjDB->get_insert('userzu',$data);
}else{
$this->MsdjDB->get_update('userzu',$id,$data);
}
admin_msg(L('plub_06'),site_url('admin/user/zu'),'ok');  //操作成功
}
...
foreach ($xid as $k=>$v) {
$this->db->query("update ".MS_SqlPrefix."userzu set xid=".$v." where id=".$k."");
}
...
if($ac=='fid'){ 
$edit['fid']=$sid;
$str=($sid==1)?'<a title="'.L('plub_24').'" href="javascript:get_cmd(\''.site_url('admin/user/zu_init').'?sid=0\',\'fid\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/ok.gif" /></a>':'<a title="'.L('plub_24').'" href="javascript:get_cmd(\''.site_url('admin/user/zu_init').'?sid=1\',\'fid\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/no.gif" /></a>';
}elseif($ac=='aid'){
$edit['aid']=$sid;
$str=($sid==1)?'<a title="'.L('plub_24').'" href="javascript:get_cmd(\''.site_url('admin/user/zu_init').'?sid=0\',\'aid\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/ok.gif" /></a>':'<a title="'.L('plub_24').'" href="javascript:get_cmd(\''.site_url('admin/user/zu_init').'?sid=1\',\'aid\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/no.gif" /></a>';
}elseif($ac=='sid'){
$edit['sid']=$sid;
$str=($sid==1)?'<a title="'.L('plub_24').'" href="javascript:get_cmd(\''.site_url('admin/user/zu_init').'?sid=0\',\'sid\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/ok.gif" /></a>':'<a title="'.L('plub_24').'" href="javascript:get_cmd(\''.site_url('admin/user/zu_init').'?sid=1\',\'sid\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/no.gif" /></a>';
}elseif($ac=='vid'){
$edit['vid']=$sid;
$str=($sid==1)?'<a title="'.L('plub_24').'" href="javascript:get_cmd(\''.site_url('admin/user/zu_init').'?sid=0\',\'vid\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/ok.gif" /></a>':'<a title="'.L('plub_24').'" href="javascript:get_cmd(\''.site_url('admin/user/zu_init').'?sid=1\',\'vid\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/no.gif" /></a>';
}elseif($ac=='mid'){
$edit['mid']=$sid;
$str=($sid==1)?'<a title="'.L('plub_24').'" href="javascript:get_cmd(\''.site_url('admin/user/zu_init').'?sid=0\',\'mid\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/ok.gif" /></a>':'<a title="'.L('plub_24').'" href="javascript:get_cmd(\''.site_url('admin/user/zu_init').'?sid=1\',\'mid\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/no.gif" /></a>';
}else{  
$edit['did']=$sid;
$str=($sid==1)?'<a title="'.L('plub_24').'" href="javascript:get_cmd(\''.site_url('admin/user/zu_init').'?sid=0\',\'did\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/ok.gif" /></a>':'<a title="'.L('plub_24').'" href="javascript:get_cmd(\''.site_url('admin/user/zu_init').'?sid=1\',\'did\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/no.gif" /></a>';
}
$this->MsdjDB->get_update('userzu',$id,$edit);
echo $str;
}
...
if(is_array($id)){
foreach ($id as $id) {
$this->db->query("update ".MS_SqlPrefix."user set zid=0 where zid=".$id."");
}
}else{
$this->db->query("update ".MS_SqlPrefix."user set zid=0 where zid=".$id."");
}

$this->MsdjDB->get_del('userzu',$id);
admin_msg(L('plub_06'),site_url('admin/user/zu'),'ok');  //操作成功
}
...
$id   = intval($this->input->get('id'));
if($id==0){
$row=$this->db->query("SELECT xid FROM ".MS_SqlPrefix."userlevel order by xid desc")->row();
$data['id']=0;
...
$row=$this->db->query("SELECT * FROM ".MS_SqlPrefix."userlevel where id=".$id."")->row(); 
if(!$row) admin_msg(L('plub_17'),'javascript:history.back();','no');  //记录不存在
...
if($id==0){
$this->MsdjDB->get_insert('userlevel',$data);
}else{
$this->MsdjDB->get_update('userlevel',$id,$data);
}
admin_msg(L('plub_06'),site_url('admin/user/level'),'ok');  //操作成功
}
...
if(is_array($id)){
foreach ($id as $id) {
$this->db->query("update ".MS_SqlPrefix."user set level=0 where level=".$id."");
}
}else{
$this->db->query("update ".MS_SqlPrefix."user set level=0 where level=".$id."");
}

$this->MsdjDB->get_del('userlevel',$id);
admin_msg(L('plub_06'),site_url('admin/user/level'),'ok');  //操作成功
}
...
$xid = $this->input->post('xid',true);
foreach ($xid as $k=>$v) {
$this->db->query("update ".MS_SqlPrefix."userlevel set xid=".$v." where id=".$k."");
}
