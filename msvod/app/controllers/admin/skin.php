...
class Skin extends msvod_Controller {
	...
	public function index()
	{
            ...
            $base_url=site_url('admin/skin').'?ac='.$ac.'&op='.$op;
				    ...
				    if (is_dir($path.$t)) {
						...
						if (file_exists($confiles)){
							...
							if(empty($ac)){
								 ...
								 $pic = Web_Path.'msvod/tpl/'.$op.'/'.$t.'/preview.jpg';
							}else{
					        ...
					        $dirs[] = array(
						        ...
						        'link' => site_url('admin/skin/show')."?ac=".$ac."&op=".$op."&dirs=".$t,
						        'ulink' => site_url('admin/skin/look')."?ac=".$ac."&op=".$op."&dirs=".$t,
						        'mrlink' => site_url('admin/skin/init')."?ac=".$ac."&op=".$op."&dirs=".$t,
						        'dellink' => site_url('admin/skin/del')."?ac=".$ac."&op=".$op."&dirs=".$t,
					        );
						}
				    }
			    }
			...
			$data['uplink']=site_url('admin/skin/upload')."?ac=".$ac."&op=".$op;
	...
	public function init()
	{
            ...
            if(empty($ac)){
			     $confiles=MSVOD.'tpl/'.$op.'/'.$dir.'/config.php';
			}else{
				 if(!empty($op)){
            ...
            if(empty($ac)){
				 ...
				 if($op=='mobile'){
	                    $users=read_file("./msvod/lib/Ms_Config.php");
	                    $users=preg_replace("/'Mobile_Skins','(.*?)'/","'Mobile_Skins','".$config['path']."'",$users);
	                    $res=write_file('./msvod/lib/Ms_Config.php', $users);
				 }elseif($op=='user'){
	                    $users=read_file("./msvod/lib/Ms_User.php");
	                    $users=preg_replace("/'User_Skins','(.*?)'/","'User_Skins','".$config['path']."'",$users);
	                    $res=write_file('./msvod/lib/Ms_User.php', $users);
				 }elseif($op=='home'){
	                    $users=read_file("./msvod/lib/Ms_Home.php");
	                    $users=preg_replace("/'Home_Skins','(.*?)'/","'Home_Skins','".$config['path']."'",$users);
	                    $res=write_file('./msvod/lib/Ms_Home.php', $users);
				 }else{
	                    $users=read_file("./msvod/lib/Ms_Config.php");
	...
	public function edit()
	{
            ...
            if($do=='add'){
                 ...
                 if (!write_file($skin_dir, $html)){
                      ...
                      admin_msg(L('plub_11'),site_url('admin/skin/show')."?ac=".$ac."&op=".$op."&dirs=".$dir."&path=".$path);
                 }
			}else{
                 $html=get_bm(read_file($skin_dir));
			     $data['savelink']=site_url('admin/skin/edit')."?ac=".$ac."&do=add&op=".$op."&dirs=".$dir."&file=".$file;
			     $data['path']=empty($ac)?str_replace(MSVOD.'tpl','',$skin_dir):str_replace(FCPATH.'plugins/'.$ac,'',$skin_dir);
	...
	public function show()
	{
				    ...
				    if (is_dir($t['server_path'])) {
						if($t['name']!='install'){
					        $dirs[] = array(
						       ...
						       'link' => site_url('admin/skin/show')."?ac=".$ac."&op=".$op."&dirs=".$dir."&path=".$path."/".$t['name'],
						       'dellink' => site_url('admin/skin/del')."?ac=".$ac."&op=".$op."&dirs=".$dir."&file=".$path."/".$t['name'],
					        );
						}
				    } else {
						...
						if($exts=='html' || $exts=='css' || $exts=='js'){
						    ...
						    $list[] = array(
							    ...
							    'link' => site_url('admin/skin/edit')."?ac=".$ac."&op=".$op."&dirs=".$dir."&file=".$path."/".$t['name'],
							    'blink' => site_url('admin/skin/copyt')."?ac=".$ac."&op=".$op."&dirs=".$dir."&file=".$path."/".$t['name'],
						        'dellink' => site_url('admin/skin/del')."?ac=".$ac."&op=".$op."&dirs=".$dir."&file=".$path."/".$t['name'],
						    );
						}
				    }
			...
			$data['addlink']=site_url('admin/skin/add')."?ac=".$ac."&op=".$op."&dirs=".$dir.$path;
			$data['path']=empty($ac)?str_replace(MSVOD.'tpl','',$skin_dir):str_replace(FCPATH.'plugins/'.$ac,'',$skin_dir);
			$data['dirs']=$dirs;
			$data['show']=$list;
			$data['uplink']=site_url('admin/skin/upload')."?ac=".$ac."&op=".$op;
			...
			$data['slink']=empty($path)?site_url('admin/skin')."?ac=".$ac."&op=".$op:site_url('admin/skin/show')."?ac=".$ac."&op=".$op."&dirs=".$dir."&path=".$spath;
	...
	public function add()
	{
			...
			if($do=='add'){
                ...
                $data['savelink']=site_url('admin/skin/add')."?ac=".$ac."&op=".$op."&do=add&dirs=".$dir;
	...
	public function copyt()
	{
            ...
            if(empty($ac)){
			     $skin_dir=MSVOD.'tpl/'.$op.'/'.$dir.'/'.$file;
			}else{
				 if(!empty($op)){
			...
			if($do=='add'){
                 ...
                 if ( ! $this->upload->do_upload()){
							...
							if ($this->cszip->extract(PCLZIP_OPT_PATH, $skin_dir, PCLZIP_OPT_REPLACE_NEWER) == 0) {
								    ...
								    //删除压缩包
									@unlink($config['upload_path'].$filename);
                                    admin_msg(L('plub_26'),site_url('admin/skin')."?ac=".$ac."&op=".$op);
							}
				 }

			}else{
				 $data['savelink']=site_url('admin/skin/upload')."?ac=".$ac."&op=".$op."&do=add";
	...
	public function yun()
	{
			header("Location: ".$this->msapp->url('skins')."");
	}
	...
	public function down()
	{
	        ...
	        if (empty($key) || empty($dir) || empty($mid) || empty($sid)){
				admin_msg('<font color=red>'.L('plub_27').'</font>',site_url('admin/skin/yun'),'no');
			}
			if($fid==1){  //板块模板
                if($sid==4){ //手机模板
                ...
                if($sid==4){ //手机模板
                      $skins_path=MSVOD.'tpl/mobile/';
                }elseif($sid==3){ //会员中心模板
                      $skins_path=MSVOD.'tpl/user/';
                }elseif($sid==2){  //会员主页模板
	                  $skins_path=MSVOD.'tpl/home/';
                }else{   //单主页模板
	                  $skins_path=MSVOD.'tpl/skins/';
                }
			}

            $data['key']=$key;
            $zip=$this->msapp->url('skins/down/'.$mid,$data);
            $zippath=FCPATH."attachment/other/skins_".$dir.".zip";
            $files_file=$this->msapp->down($zip,$zippath);
    	    if($files_file=='-1') admin_msg(L('plub_28'),site_url('admin/skin/yun'),'no');
    	    if($files_file=='-2') admin_msg(L('plub_29'),site_url('admin/skin/yun'),'no');
    	    if($files_file=='-3') admin_msg(L('plub_30'),site_url('admin/skin/yun'),'no');
    	    if($files_file=='10001') admin_msg(L('plub_31'),site_url('admin/skin/yun'),'no');
    	    if($files_file=='10002') admin_msg(L('plub_32'),site_url('admin/skin/yun'),'no');
	        if(filesize($zippath) == 0) admin_msg(L('plub_31'),site_url('admin/skin/yun'),'no');
		    ...
		    if ($this->cszip->extract(PCLZIP_OPT_PATH, $skins_path, PCLZIP_OPT_REPLACE_NEWER) == 0) {
                    	@unlink($zippath);
                        admin_msg(L('plub_33'),site_url('admin/skin/yun'),'no');
		    }else{

		               if (!file_exists($skins_path.$dir.'/config.php')) {
			                @unlink($zippath);
                            admin_msg(L('plub_34'),site_url('admin/skin/yun'),'no');
					   }
			           @unlink($zippath);
                       admin_msg(L('plub_35'),site_url('admin/skin').'?ac='.$mold);
			}
	}
	...
	{      
            ...
            //获得当前模型字段信息
		    $query = $this->db->query("SHOW FULL FIELDS FROM ".MS_SqlPrefix.$data['mx']."");
	...
	public function update()
	{
	        ...
	        if (empty($key) || empty($dir)){
                  ...
                  $data['mid']=$mid;
                  header("Location: ".$this->msapp->url('skins/update',$data)."");

			}else{  //下载

                  $data['key']=$key;
                  $zip=$this->msapp->url('skins/update/'.$mid,$data);
                  $zippath=FCPATH."attachment/other/skins_".$dir."_update.zip";
                  $files_file=$this->msapp->down($zip,$zippath);
    	          if($files_file=='-1') admin_msg(L('plub_28'),site_url('admin/skin/yun'),'no');
    	          if($files_file=='-2') admin_msg(L('plub_29'),site_url('admin/skin/yun'),'no');
    	          if($files_file=='-3') admin_msg(L('plub_30'),site_url('admin/skin/yun'),'no');
    	          if($files_file=='10001') admin_msg(L('plub_36'),site_url('admin/skin/yun'),'no');
    	          if($files_file=='10002') admin_msg(L('plub_37'),site_url('admin/skin/yun'),'no');
    	          if($files_file=='10003') admin_msg(L('plub_38'),site_url('admin/skin/yun'),'no');
	              if(filesize($zippath) == 0) admin_msg(L('plub_39'),site_url('admin/skin/yun'),'no');
		          ...
		          if ($this->cszip->extract(PCLZIP_OPT_PATH, $plub_path, PCLZIP_OPT_REPLACE_NEWER) == 0) {
			           ...
			           @unlink($zippath);
                       admin_msg(L('plub_41'),site_url('admin/skin').'?ac='.$mold);
				  }
			}
	}
}
