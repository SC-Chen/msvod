...
class Upgrade extends msvod_Controller {
	...
	function __construct() {
		...
		$this->_filearr = array('.', 'packs', 'msvod'); //MD5效验目录
	}

	//系统升级
	public function index() {
		$msvod_update = htmlall($this->_upgrade . 'ajax/update?charset=utf-8&version=' . MS_Version . '&update=' . MS_Uptime);
		$data['msvod_update'] = json_decode($msvod_update, 1);
	...
	public function init() {
		...
		//当前版本内容
		$versiondata = read_file(MSVOD . 'lib/Ms_Version.php');
		...
		if ($this->copyfailnum > 0) {
			//如果失败，恢复当前版本
			write_file(MSVOD . 'lib/Ms_Version.php', $versiondata);
			...
			echo '&nbsp;&nbsp;<b style="color:red">' . L('plub_09') . '&nbsp;&nbsp;<a href="' . site_url('admin/upgrade') . '">' . L('plub_10') . '</a></b>';
			@unlink($zippath);
			exit;
		}
		...
		deldir($dircache);
		//成功提示
		admin_msg(L('plub_11'), site_url('admin/upgrade'));
	}
	...
	public function look() {
		...
		//判断重要文件，不能查看
		if (strpos(strtolower($path), 'msvod/lib') !== FALSE) {
			die(L('plub_14'));
		}
	...
	public function checkfile() {
		if (!empty($_GET['ok'])) {
			...
			//读取msvod接口
			$msvod_md5 = htmlall($this->_upgrade . 'ajax/filemd5?update=' . MS_Uptime);
			$msvod_md5_arr = json_decode($msvod_md5, 1);

			if (!empty($msvod_md5_arr)) {
				//计算数组差集
				$diff = array_diff($msvod_md5_arr, $this->md5_arr);

				//丢失文件列表
				$lostfile = array();
				foreach ($msvod_md5_arr as $k => $v) {
				...
				//未知文件列表
				$data['unknowfile'] = array_diff(array_keys($this->md5_arr), array_keys($msvod_md5_arr));
			}
			$this->load->view('upgrade_md5.html', $data);

		} else {
			admin_msg(L('plub_15'), site_url('admin/upgrade/checkfile') . '?ok=1', 'ok'); //操作成功
		}
	}
	...
	private function filemd5($path = '') {
		...
		if (is_dir($path)) {
			...
			if (dirname($path) == '.' || (isset($dir_arr[1]) && in_array($dir_arr[1], $this->_filearr))) {
				if (strpos($path, 'msvod/lib/') === FALSE && strpos($path, 'packs/install/') === FALSE) {
					if (strpos($path, 'msvod/tpl/') !== FALSE) {
						if (strpos($path, 'msvod/tpl/admin/') !== FALSE) {
				...
				if ($exts == 'sql') {
					...
					if (!empty($sql)) {
						...
						foreach ($sqlarr as $sqls) {
							$this->MsdjDB->get_table(str_replace('{prefix}', MS_SqlPrefix, $sqls));
						}
					}
				}
			}
