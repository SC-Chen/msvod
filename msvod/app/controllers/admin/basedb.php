...
class Basedb extends msvod_Controller {
	...
	{
			$data['tables']=$this->db->query("SHOW TABLE STATUS FROM `".MS_Sqlname."`")->result();
	...
	public function optimize()
	{
			...
			if(!empty($error)){
                admin_msg(vsprintf(L('plub_02'),array(implode(',', $error))),site_url('admin/basedb'),'no');
			}else{
                admin_msg(L('plub_03'),site_url('admin/basedb'));
			}
	}
	...
	public function repair()
	{
			...
			if(!empty($error)){
                ...
                admin_msg(L('plub_05'),site_url('admin/basedb'));
			}
	}
	...
	public function backup()
	{
            ...
            $res=$this->MsdjBackup->backup($tables);
			if($res){
                 $this->MsdjBackup->bkdata($size); 
                 admin_msg(L('plub_07'),site_url('admin/basedb/restore'));
			}else{
	...
	public function restore_save()
	{
			...
			$this->MsdjBackup->restore($dirs);
            admin_msg(L('plub_10'),site_url('admin/basedb/restore'));
	}
	...
	public function fields()
	{
			...
			$data['table'] = $this->MsdjBackup->repair($table);
	...
	public function del()
	{
            ...
            admin_msg(L('plub_13'),site_url('admin/basedb/restore'));
	}
}
