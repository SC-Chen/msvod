...
class Plugins extends msvod_Controller {
...
//获取版块目录
	public function index() {
		...
		$data['url'] = site_url('admin/admin/plugins') . "?";
...
//配置
	public function setting() {
		...
		$data['dir'] = $dir;
		$row = $this->db->select("ak,name")->get_where(MS_SqlPrefix . "plugins", ["dir"=>$dir])->row();
		$arrs = unarraystring(sys_auth($row->ak, 'D'));
		$data['key'] = (empty($arrs) || empty($arrs['key']) || $arrs['key'] == '0') ? '' : $arrs['key'];
		if ($data['model']['name'] != $row->name) {
...
//配置保存
	public function setting_save() {
		...
		$row = $this->db->query("SELECT ak,name FROM " . MS_SqlPrefix . "plugins where dir='" . $dir . "'")->row();
		if (!empty($name) && $name != $row->name) {
			$this->db->query("update " . MS_SqlPrefix . "plugins set name='" . $name . "' where dir='" . $dir . "'");
		}
		$arrs = unarraystring(sys_auth($row->ak, 'D'));
		if (empty($key)) {
			$key = '0';
...
//判断开启二级域名
			global $_MS_Domain;
			if ($Ym_Mode == 1) {
				$_MS_Domain[$dir] = $Ym_Url;
				arr_file_edit($_MS_Domain);
			} else {
				if (arr_key_value($_MS_Domain, $dir)) {
					unset($_MS_Domain[$dir]);
					arr_file_edit($_MS_Domain);
				}
			}
...
//伪静态模式，写入URL路由
			...
			admin_msg(L('plub_05'), site_url('admin/plugins'), 'ok'); //修改完成

		} else {
			admin_msg(L('plub_06'), site_url('admin/plugins'), 'no'); //板块不存在
		}
	}
...
//安装
	public function install() {
		...
		if (is_dir(FCPATH . 'plugins/' . $dir)) {
			...
			if (!file_exists(FCPATH . 'plugins/' . $dir . '/config/install.php') || $model['mid'] == '') {
				admin_msg(L('plub_07'), 'javascript:history.back();');
			}

			$row = $this->db->query("SELECT id FROM " . MS_SqlPrefix . "plugins where dir='" . $dir . "'")->row();
			if ($row) {
				admin_msg(L('plub_08'), site_url('admin/plugins'), 'no'); //板块已经安装
			} else {

//判断数据库
				if (is_array($SQLDB)) {
					foreach ($SQLDB as $sql) {
						$this->db->query(str_replace('{prefix}', MS_SqlPrefix, $sql));
					}
				} else {
					$this->db->query(str_replace('{prefix}', MS_SqlPrefix, $SQLDB));
				}
				...
				$this->MsdjDB->get_insert('plugins', $add);

				$data['dir'] = $dir;
				$data['mid'] = $model['mid'];
				$installurl = $this->msapp->url('plub/installs', $data);
				echo '<script src="' . $installurl . '"></script>';
				echo '<script>parent.refresh();</script>'; //刷新头部和边栏
				admin_msg(L('plub_09'), site_url('admin/plugins'), 'ok'); //板块安装成功
			}
		} else {
			admin_msg(L('plub_06'), site_url('admin/plugins'), 'no'); //板块不存在
		}
...
//卸载
	public function uninstall() {
		...
		if (is_dir(FCPATH . 'plugins/' . $dir)) {
			$row = $this->db->query("SELECT id FROM " . MS_SqlPrefix . "plugins where dir='" . $dir . "'")->row();
			if (!$row) {
				admin_msg(L('plub_10'), site_url('admin/plugins'), 'no'); //板块还没有安装
			} else {
...
//删除数据库记录
				$this->db->query("delete from " . MS_SqlPrefix . "plugins where dir='" . $dir . "'");

				$data['dir'] = $dir;
				$data['mid'] = $model['mid'];
				$uninstallurl = $this->msapp->url('plub/uninstall', $data);
				echo '<script src="' . $uninstallurl . '"></script>';
				echo '<script>parent.refresh();</script>'; //刷新头部和边栏
				admin_msg(L('plub_11'), site_url('admin/plugins'), 'ok'); //板块卸载成功
			}
		} else {
			admin_msg(L('plub_06'), site_url('admin/plugins'), 'no'); //板块不存在
		}
...
//先删除数据表
				...
				if (is_array($SQLDB1)) {
					...
					$this->db->query(str_replace('{prefix}', MS_SqlPrefix, $SQLDB1));
				}
...
//在重新安装数据表
				...
				if (is_array($SQLDB2)) {
					...
					$this->db->query(str_replace('{prefix}', MS_SqlPrefix, $SQLDB2));
				}
				admin_msg(L('plub_12'), site_url('admin/plugins'), 'ok'); //版块数据清空成功
			}
		} else {
			admin_msg(L('plub_06'), site_url('admin/plugins'), 'no'); //板块不存在
		}
...
//删除
	public function del() {
		...
		admin_msg(L('plub_13'), site_url('admin/plugins'), 'ok'); //版块删除成功
	}

//云平台
	public function yun() {
		header("Location: " . $this->msapp->url('plub') . "");
	}
...
//在线安装
	public function down() {
		...
		if (empty($mid) || empty($dir)) {
			...
			admin_msg(L('plub_14'), site_url('admin/plugins'), 'no');

		} else {
			//下载
			$data['key'] = $key;
			$zip = $this->msapp->url('plub/down/' . $mid, $data);
			$zippath = FCPATH . "attachment/other/plugins_" . $dir . ".zip";
			$files_file = $this->msapp->down($zip, $zippath);
			if ($files_file == '-1') {
				admin_msg(L('plub_15'), site_url('admin/plugins'), 'no');
			}

			if ($files_file == '-2') {
				admin_msg(L('plub_16'), site_url('admin/plugins'), 'no');
			}

			if ($files_file == '-3') {
				admin_msg(L('plub_17'), site_url('admin/plugins'), 'no');
			}

			if ($files_file == '10001') {
				admin_msg(L('plub_18'), site_url('admin/plugins'), 'no');
			}

			if ($files_file == '10002') {
				admin_msg(L('plub_19'), site_url('admin/plugins'), 'no');
			}

			if (filesize($zippath) == 0) {
				admin_msg(L('plub_20'), site_url('admin/plugins'), 'no');
			}
...
//尝试解压覆盖
			if ($this->cszip->extract(PCLZIP_OPT_PATH, $plub_path, PCLZIP_OPT_REPLACE_NEWER) == 0) {
				...
				if (!file_exists($plub_path . $dir . '/config/setting.php')) {
					...
					admin_msg(L('plub_22'), site_url('admin/plugins'), 'no');
				}
				@unlink($zippath);
				admin_msg(L('plub_23'), site_url('admin/plugins'));
			}
		}
	}
...
//在线升级
	public function update() {
		...
		if (empty($key) || empty($dir)) {
			...
			$data['mid'] = $markid;
			$data['v'] = $v;
			header("Location: " . $this->msapp->url('plub/update', $data) . "");

		} else {
			//下载

			$zip = $this->msapp->url('plub/update') . '&code=gbk&key=' . $key;
			...
			if ($files_file == '10001') {
				admin_msg(L('plub_24'), site_url('admin/plugins'), 'no');
			}

			if ($files_file == '10002') {
				admin_msg(L('plub_25'), site_url('admin/plugins'), 'no');
			}

			if ($files_file == '10003') {
				admin_msg(L('plub_26'), site_url('admin/plugins'), 'no');
			}

			if (filesize($zippath) == 0) {
				admin_msg(L('plub_27'), site_url('admin/plugins'), 'no');
			}
...
//尝试解压覆盖
			if ($this->cszip->extract(PCLZIP_OPT_PATH, $plub_path, PCLZIP_OPT_REPLACE_NEWER) == 0) {
				...
				@unlink($zippath);
				admin_msg(L('plub_29'), site_url('admin/plugins'));
			}
		}
	}
...
//板块后台权限划分
	public function role() {
		...
		if (is_dir(FCPATH . 'plugins/' . $dir)) {
			...
			$data['dir'] = $dir;
			$row = $this->db->query("SELECT app FROM " . MS_SqlPrefix . "adminzu where id='" . $id . "'")->row();
...
//板块后台权限修改
	public function role_save() {
		...
		$this->MsdjDB->get_update('adminzu', $id, $data);
