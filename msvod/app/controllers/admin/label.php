...
class Label extends msvod_Controller {
	...
	public function index()
	{
            ...
            $sql_string = "SELECT * FROM ".MS_SqlPrefix."label order by addtime desc";
	        $query = $this->db->query($sql_string); 
	        $total = $query->num_rows();

            $base_url = site_url('admin/label/index');
	        $per_page = 15; 
	...
	public function edit()
	{
            ...
            $row=$this->db->query("SELECT * FROM ".MS_SqlPrefix."label where id=".$id."")->row(); 
			if(!$row) admin_msg(L('plub_01'),site_url('admin/label'),'no');  //记录不存在
	...
	public function save()
	{
			...
			if($id==0){ //新增
                 $row=$this->db->query("SELECT id FROM ".MS_SqlPrefix."label where name='".$data['name']."'")->row(); 
			     if($row) admin_msg(L('plub_03'),'javascript:history.back();','no');  //标签名称已经存在
                 $this->MsdjDB->get_insert('label',$data);
			}else{
                 $this->MsdjDB->get_update('label',$id,$data);
			}
            admin_msg(L('plub_04'),site_url('admin/label'),'ok');  //操作成功
	}
	...
	public function js()
	{
            ...
            $sql_string = "SELECT * FROM ".MS_SqlPrefix."ads order by addtime desc";
	        $query = $this->db->query($sql_string); 
	        $total = $query->num_rows();

            $base_url = site_url('admin/label/js');
	        $per_page = 15; 
            $totalPages = ceil($total / $per_page); // 总页数
	        $data['nums'] = $total;
            if($total<$per_page){
	...
	{
            $id   = intval($this->input->get('id'));
            $row=$this->db->query("SELECT * FROM ".MS_SqlPrefix."ads where id=".$id."")->row(); 
			if(!$row) admin_msg(L('plub_01'),site_url('admin/label/js'),'no');  //记录不存在
	...
	public function js_save()
	{
			...
			if($id==0){ //新增
                 $row=$this->db->query("SELECT id FROM ".MS_SqlPrefix."ads where name='".$data['name']."'")->row(); 
			     if($row) admin_msg(L('plub_03'),'javascript:history.back();','no');  //标签名称已经存在
                 $row=$this->db->query("SELECT id FROM ".MS_SqlPrefix."ads where js='".$data['js']."'")->row(); 
			     if($row) admin_msg(L('plub_08'),'javascript:history.back();','no');  //标签JS文件名已经存在

                 $this->MsdjDB->get_insert('ads',$data);
			}else{
                 $this->MsdjDB->get_update('ads',$id,$data);
			}
            ...
            admin_msg(L('plub_04'),site_url('admin/label/js'),'ok');  //操作成功
	}
	...
	public function js_del()
	{
			...
			if(is_array($id)){
			   foreach ($id as $ids) {
				    $row=$this->db->query("SELECT js FROM ".MS_SqlPrefix."ads where id='".$ids."'")->row();
				    if($row){
				    ...
				    $row=$this->db->query("SELECT js FROM ".MS_SqlPrefix."ads where id='".$id."'")->row();
				    if($row){
                        $jsurl='.'.Web_Path.'attachment/js/'.$row->js.'.js';
				        @unlink($jsurl);
				    }
			}
			$this->MsdjDB->get_del('ads',$id);
            admin_msg(L('plub_06'),'javascript:history.back();','ok');  //操作成功
	}
	...
	public function page()
	{
            ...
            $sql_string = "SELECT * FROM ".MS_SqlPrefix."page order by addtime desc";
	        $query = $this->db->query($sql_string); 
	        $total = $query->num_rows();

            $base_url = site_url('admin/label/page');
	        $per_page = 15; 
            $totalPages = ceil($total / $per_page); // 总页数
	        $data['nums'] = $total;
            if($total<$per_page){
	...
	{
            $id   = intval($this->input->get('id'));
            $row=$this->db->query("SELECT * FROM ".MS_SqlPrefix."page where id=".$id."")->row(); 
			if(!$row) admin_msg(L('plub_01'),site_url('admin/label/page'),'no');  //记录不存在
	...
	public function page_save()
	{
			...
			if($id==0){ //新增
                 $row=$this->db->query("SELECT id FROM ".MS_SqlPrefix."page where name='".$data['name']."'")->row(); 
			     if($row) admin_msg(L('plub_03'),'javascript:history.back();','no');  //标签名称已经存在

                 $row=$this->db->query("SELECT id FROM ".MS_SqlPrefix."page where url='".$data['url']."'")->row(); 
			     if($row) admin_msg(L('plub_13'),'javascript:history.back();','no');  //标签URL地址已经存在

                 $this->MsdjDB->get_insert('page',$data);
			}else{
                 $this->MsdjDB->get_update('page',$id,$data);
			}
            admin_msg(L('plub_04'),site_url('admin/label/page'),'ok');  //操作成功
	}
	...
	{
            $id=intval($this->input->get('id'));
			$row=$this->MsdjDB->get_row_arr('page','url,html,name',$id);
			if($row){
                    $path=str_replace("//","/",FCPATH.$row['url']);
					$this->load->model('MsdjTpl');
					$neir=$this->MsdjTpl->page($row,true);
	...
	public function page_del()
	{
			...
			if(is_array($id)){
			   foreach ($id as $ids) {
				    $row=$this->db->query("SELECT sid,url FROM ".MS_SqlPrefix."page where id='".$ids."'")->row();
				    if($row && $row->sid==1){
                        $html='.'.$row->url;
				        @unlink($html);
				    }
				    ...
				    $row=$this->db->query("SELECT sid,url FROM ".MS_SqlPrefix."page where id='".$id."'")->row();
				    if($row && $row->sid==1){
                        $html='.'.$row->url;
				        @unlink($html);
				    }
	...
	{
		    //所有板块
            $sql_string = "SELECT dir,name FROM ".MS_SqlPrefix."plugins order by id asc";
            $query = $this->db->query($sql_string); 
	...
	public function deldata_save()
	{
            ...
            $this->db->query("delete from ".MS_SqlPrefix.$table." ");
			//修复主键ID
			if($ids=='ok'){
			     $this->db->query("TRUNCATE TABLE ".MS_SqlPrefix.$table." ");
			}
	...
	public function editdata_save()
	{
            ...
            $sql="select id,".$field." from ".MS_SqlPrefix.$table." where ".$field." like '%".$old."%'";
			$result=$this->db->query($sql);
			foreach ($result->result() as $row) { 
				$newneir=str_replace($old,$new,$row->$field);
				$sql3="update ".MS_SqlPrefix.$table." set ".$field."='".$newneir."' where id=".$row->id."";
				$this->db->query($sql3);
			}
	...
	{
			$this->load->model('MsdjBackup');
			$table = $this->input->get('table',true);
			$table = $this->MsdjBackup->repair(MS_SqlPrefix.$table);
