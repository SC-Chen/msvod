...
{
$sid=intval($this->input->get_post('sid',TRUE)); //版块类型,1为视频，2为资讯，3为电影，4为演员
if($sid==0) $sid=1;
echo "<select name='list'>";
if($sid==1){
$sqlstr="select id,name from ".MS_SqlPrefix."video_list order by xid asc";
}elseif($sid==2){
$sqlstr="select id,name from ".MS_SqlPrefix."news_list order by xid asc";
}elseif($sid==3){
$sqlstr="select id,name from ".MS_SqlPrefix."vod_list order by xid asc";
}else{
$sqlstr="select id,name from ".MS_SqlPrefix."singer_list order by xid asc";
}
$result=$this->MsdjDB->db->query($sqlstr);
...
$row=$this->db->query("select id from ".MS_SqlPrefix."video where name='".$music['name']."'")->row();
if($row){
echo "数据已存在,跳过";
}else{
$did=$this->MsdjDB->get_insert('video',$music);
if($did>0){
echo "增加信息成功";
}else{
echo "增加信息失败";
...
$row=$this->db->query("select id from ".MS_SqlPrefix."news where name='".$news['name']."'")->row();
if($row){
echo "数据已存在,跳过";
}else{
...
$vod['addtime']=time();
//必填字段
$vod['name']=$this->input->get_post('name', TRUE, TRUE);//电影名称
$vod['cid']=intval($this->input->get_post('cid'));//电影分类ID
...
$row=$this->db->query("select id,purl,durl from ".MS_SqlPrefix."vod where name='".$vod['name']."'")->row();
if($row){
$s=0;
if(empty($row->purl)){
$vod2['purl']=$vod['purl'];
...
$vod2['purl']=$row->purl.'#msvod#'.$vod['purl'];
echo "数据存在，新增一组播放";
$s++;
}
}
...
$vod2['durl']=$row->durl.'#msvod#'.$vod['durl'];
echo "数据存在，新增一组下载";
$s++;
}
}
if($s>0){
$this->MsdjDB->get_update('vod',$row->id,$vod2);
}else{
echo "数据已存在,跳过";
}
}else{
$did=$this->MsdjDB->get_insert('vod',$vod);
if($did>0){
echo "增加信息成功";
}else{
echo "增加信息失败";
...
//判断数据是否相同
$row=$this->db->query("select id from ".MS_SqlPrefix."singer where name='".$singer['name']."'")->row();
if($row){
$this->MsdjDB->get_update('singer',$row->id,$singer);
echo "数据存在，资料修改成功";
}else{
$did=$this->MsdjDB->get_insert('singer',$singer);
if($did>0){
echo "增加信息成功";
}else{
echo "增加信息失败";
