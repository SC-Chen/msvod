...
class Web extends msvod_Controller {
	...
	public function picsave()
	{
		   ...
		   $bgpic=getzd('user','bgpic',$_SESSION['msvod__id']);
		   //删除原来的图片
		   $this->load->library('msup');
		   $this->msup->del($bgpic,'bgpic'); //删除附件
           ...
           echo '<script type="text/javascript" src="'.Web_Path.'packs/js/msvod.js"></script><script type="text/javascript">
    ...
    public function upload($ac) {  
		   ...
		   //判断上传方式
           $this->load->library('msup');
		   $res=$this->msup->up($file_path,$new_file_name);
		   if($res){
