...
class Msg extends msvod_Controller {
	...
	public function index()
	{
			...
			//URL地址
		    $url='msg/index/'.$op;
            $sqlstr = "select * from ".MS_SqlPrefix."msg where uida=".$_SESSION['msvod__id'];
            if($op=='xt'){
                  $sqlstr.= " and uidb=0";
			}
            if($op=='wd'){
	...
	public function show()
	{
			...
			//URL地址
		    $url='msg/show/'.$id;
		    $row=$this->MsdjDB->get_row_arr('msg','*',$id);
			if(!$row) msg_url(L('msg_03'),'javascript:history.back();');
			if($row['uida']!=$_SESSION['msvod__id']) msg_url(L('msg_04'),'javascript:history.back();');
            if($row['did']==0){//变更为已读
                 $this->db->query("update ".MS_SqlPrefix."msg set did=1 where id=".$id."");
			}
			//当前会员
		    $rowu=$this->MsdjDB->get_row_arr('user','*',$_SESSION['msvod__id']);
			if(empty($row['nichen'])) $row['nichen']=$row['name'];
			//装载模板
			$title=L('msg_05');
			$ids['uid']=$_SESSION['msvod__id'];
			$ids['uida']=$_SESSION['msvod__id'];
            $Mark_Text=$this->MsdjTpl->user_list($rowu,$url,1,$tpl,$title,'','',$ids,true,false);
            $Mark_Text=$this->skins->msvod_skins('msg',$Mark_Text,$Mark_Text,$row);//解析当前消息标签
			//会员版块导航
			$Mark_Text=$this->skins->msvodumenu($Mark_Text,$_SESSION['msvod__id']);
            $Mark_Text=$this->skins->labelif($Mark_Text);
			echo $Mark_Text;
	...
	public function add()
	{
			...
			$name='';
			if($id>0){
		        $rows=$this->MsdjDB->get_row('msg','*',$id);
			...
			//检测发表权限
			$rowz=$this->MsdjDB->get_row('userzu','mid',$row['zid']);
			if($rowz->mid==0){
	...
	public function save()
	{
			...
			$rowu=$this->MsdjDB->get_row('userzu','mid',$zuid);
			if($rowu->mid==0){
                 msg_url(L('msg_07'),'javascript:history.back();');
			}
	...
	public function del()
	{
			...
			if($op=='yue'){ //前一个月
				$times=time()-86400*30;
                $this->db->query("delete from ".MS_SqlPrefix."msg where uida=".$_SESSION['msvod__id']." and addtime<".$times."");
			}elseif($op=='all'){  //全部
                $this->db->query("delete from ".MS_SqlPrefix."msg where uida=".$_SESSION['msvod__id']."");
			}else{ //按ID
		        $id=intval($this->uri->segment(5)); //ID
			    if($id==0) msg_url(L('msg_02'),'javascript:history.back();');
                $this->db->query("delete from ".MS_SqlPrefix."msg where uida=".$_SESSION['msvod__id']." and id=".$id."");
			}
