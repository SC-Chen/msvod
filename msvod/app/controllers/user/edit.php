...
class Edit extends msvod_Controller {
	...
	public function save()
	{
            ...
            //判断昵称是否注册
			$nichen=$this->db->query("select id from ".MS_SqlPrefix."user where nichen='".$userinfo['nichen']."' and id!=".$_SESSION['msvod__id']."")->row();
			if($nichen){
                   msg_url(L('edit_07'),'javascript:history.back();');
			}

            //判断邮箱是否注册
			$email=$this->db->query("select id from ".MS_SqlPrefix."user where email='".$userinfo['email']."' and id!=".$_SESSION['msvod__id']."")->row();
			if($email){
                   msg_url(L('edit_08'),'javascript:history.back();');
			}

            //判断手机是否注册
			$tel=$this->db->query("select id from ".MS_SqlPrefix."user where tel='".$userinfo['tel']."' and id!=".$_SESSION['msvod__id']."")->row();
			if($tel){
	...
	{
		    $uid=isset($_SESSION['msvod__id'])?intval($_SESSION['msvod__id']):intval($this->cookie->get_cookie('user_id'));
		    ...
		    if (!empty($tempFile) && $uid>0) {
				   ...
				   $res=$this->msup->up($filename,$picname);
				   if(!$res){
						@unlink($filename);
	                    exit('UploadPicError');
				   }
				   ...
				   if($pic!=$filepath){
                       $this->msup->del($pic,'logo');
				   }

                   //写入数据库
                   $this->db->query("update ".MS_SqlPrefix."user set logo='".$filepath."' where id=".$uid."");
	...
	public function pass_save()
	{
            ...
            //判断原密码
			$row=$this->db->query("select code,pass from ".MS_SqlPrefix."user where id=".$_SESSION['msvod__id']."")->row();
	...
	public function open()
	{
            ...
            $Mark_Text=$this->MsdjTpl->user_list($row,$url,1,$tpl,$title,'id','',$ids,true,false);
			//第三方登录模式
			$Mark_Text=str_replace("[user:appmode]",MS_Appmode,$Mark_Text);
            //判断是否绑定
			$qq=$wb=$bd=$rr=$kx=$db=0;
			$result=$this->db->query("select cid from ".MS_SqlPrefix."useroauth where uid=".$_SESSION['msvod__id']." order by id desc");
