...
class Pass extends msvod_Controller {
	...
	public function index()
	{
			...
			if(!empty($username) && !empty($key)){
			         ...
			         $row=$this->MsdjDB->get_row('user','id,name,pass,email',$username,'name');
			         if(!$row){
			...
			$Mark_Text=str_replace("{msvod:title}",L('pass_03')." - ".Web_Name,$Mark_Text);
			//判断验证码开关
	...
	public function save()
	{
            ...
            if(empty($username)){
                ...
                //可以用会员名、邮箱来进行登入
                $sqlu="SELECT code,email,pass,id,name FROM ".MS_SqlPrefix."user where name='".$username."' and email='".$useremail."'";
	...
	public function edit()
	{
			...
			$row=$this->MsdjDB->get_row('user','id,name,code,pass,email',$username,'name');
			if(!$row){
                   msg_url(L('pass_12'),'javascript:window.close();');
			}
			if($key != md5($row->id.$row->name.$row->pass.$row->email.substr(time(),0,-6))){
