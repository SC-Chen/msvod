...
class Open extends msvod_Controller {
	...
	public function callback()
	{
            ...
            if(!empty($_SESSION['denglu__id'])){
                 ...
                 if($this->MsdjUser->User_Login(1)){ 
	                  $updata['uid']   = $_SESSION['msvod__id'];
                      $this->MsdjDB->get_update ('useroauth',$_SESSION['denglu__id'],$updata);
			...
			if($uid>0){ //已经存在则登录
                 ...
                 $sqlu="SELECT code,email,pass,sid,yid,id,name,lognum,cion,vip,logtime,viptime,zid,zutime FROM ".MS_SqlPrefix."user where id=".$uid."";
		         ...
		         if($row){
                           if($row->sid==1){
                                   ...
                                   $this->MsdjEmail->send($row->email,$title,$content);
	                           ...
	                           $updata['logip']   = getip();
	                           $updata['logms']   = time();
                               $this->MsdjDB->get_update ('user',$row->id,$updata);
							   ...
							   $add['logintime']=time();
							   $add['useragent']=$agent;
							   $this->MsdjDB->get_insert('user_log',$add);

                               $_SESSION['msvod__id']    = $row->id;
                               $_SESSION['msvod__name']  = $row->name;
                               $_SESSION['msvod__login'] = md5($row->name.$row->pass);
			     ...
			     $Mark_Text=str_replace("{msvod:title}",L('open_08')." - ".Web_Name,$template);
	...
	public function log_save()
	{
            ...
            if(empty($username)){
                ...
                //可以用会员名、邮箱来进行登入
                $sqlu="SELECT code,email,pass,sid,yid,id,name,lognum,cion,vip,logtime,viptime FROM ".MS_SqlPrefix."user where name='".$username."' or email='".$username."'";
		        $row=$this->db->query($sqlu)->row();
		        if(!$row){
	                 ...
	                 if(User_Uc_Mode==1){
	                            ...
	                            if(intval($uid[0]) > 0 ) {  //UC存在则新增会员
	                                    ...
	                                    $user['logms'] = time();
                                        $user['addtime'] = time();
                                        $res=$this->MsdjDB->get_insert('user',$user);
                                        if(intval($res) > 0 ) {

											 //修改第三方登录UID
											 $this->db->query("update ".MS_SqlPrefix."useroauth set uid=".$res." where id=".$_SESSION['denglu__id']."");
						                     //摧毁登录SESSION
						                     unset($_SESSION['denglu__id'],$_SESSION['denglu__name'],$_SESSION['denglu__logo']);
										     ...
										     if(User_Uc_Fun==0){  //不需要激活
                                                   ...
                                                   $_SESSION['msvod__id']    = $res;
                                                   $_SESSION['msvod__name']  = $username;
                                                   $_SESSION['msvod__login'] = md5($username.$user['pass']);
                                                   ...
                                                   $content = getmsgto(User_RegEmailContent,$Msgs);
												   $this->load->model('MsdjEmail');
                                                   $this->MsdjEmail->send($user['email'],$title,$content);
                         ...
                         msg_url('账号不存在','javascript:history.back();');  //账号不存在
                           ...
                           if($row->pass!=md5(md5(trim($userpass)).$row->code)){
						       ...
						       //修改第三方登录UID
						       $this->db->query("update ".MS_SqlPrefix."useroauth set uid=".$row->id." where id=".$_SESSION['denglu__id']."");
						       //摧毁登录SESSION
						       unset($_SESSION['denglu__id'],$_SESSION['denglu__name'],$_SESSION['denglu__logo']);
	...
	public function reg_save()
	{
            ...
            //判断同一IP注册时间限制
			if(User_RegIP>0){
			    $row=$this->db->query("SELECT addtime FROM ".MS_SqlPrefix."user where regip='".$userinfo['regip']."' order by id desc")->row();
            ...
            //判断用户名是否注册
			$username=$this->MsdjDB->get_row('user','id',$userinfo['name'],'name');
			if($username){
                   msg_url(L('reg_10'),'javascript:history.back();');
			}

            //判断邮箱是否注册
			$useremail=$this->MsdjDB->get_row('user','id',$userinfo['email'],'email');
			if($useremail){
		    ...
		    if(!empty($userinfo['tel'])){
                  ...
                  //判断手机号码是否注册
			      $usertel=$this->MsdjDB->get_row('user','id',$userinfo['tel'],'tel');
			      if($usertel){
            ...
            $regid=$this->MsdjDB->get_insert('user',$userinfo);
			if(intval($regid)==0){
				 msg_url(L('reg_17'),'javascript:history.back();');
			}

			//修改第三方登录UID
			$this->db->query("update ".MS_SqlPrefix."useroauth set uid=".$regid." where id=".$_SESSION['denglu__id']."");
			...
			if(User_RegEmailFun==1) { //发送激活邮件
                      ...
                      $content = getmsgto(User_RegEmailContent,$Msgs);
                      $this->MsdjEmail->send($userinfo['email'],$title,$content);
			}
			...
			if(User_RegMsgFun==1){
                      ...
                      $addmsg['addtime'] = time();
                      $this->MsdjDB->get_insert('msg',$addmsg);
            ...
            if($userinfo['yid']==0){ //登录
	                  ...
	                  $updata['logip']   = getip();
	                  $updata['logms']   = time();
                      $this->MsdjDB->get_update ('user',$regid,$updata);
                      ...
                      $_SESSION['msvod__id']    = $regid;
                      $_SESSION['msvod__name']  = $userinfo['name'];
                      $_SESSION['msvod__login'] = md5($userinfo['name'].$userinfo['pass']);
