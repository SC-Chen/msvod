...
class Reg extends msvod_Controller {
...
$Mark_Text=str_replace("{msvod:title}",L('reg_01')." - ".Web_Name,$template);
...
$row=$this->MsdjDB->get_row_arr('user','*',$id);
...
if(!empty($usertid)){
$user=$this->MsdjDB->get_row_arr('user','*',$usertid);
if($user['dlrz']==2){
...
$daili['dljb']=1;
$daili['addtime']=time();
$this->MsdjDB->get_insert('daili_xiaji',$daili);
if($user['dlid']>0){
$dailis['uid']=$regid;
$dailis['dlid']=$user['dlid'];
$dailis['dljb']=2;
$dailis['addtime']=time();
$this->MsdjDB->get_insert('daili_xiaji',$dailis);
}
$dailis=$this->db->query("select * from ".MS_SqlPrefix."user where id=".$user['dlid']."")->row();
if($dailis->dlid>0){
$dailia['uid']=$regid;
$dailia['dlid']=$dailis->dlid;
$dailia['dljb']=3;
$dailia['addtime']=time();
$this->MsdjDB->get_insert('daili_xiaji',$dailia);
}
}
//摧毁token
get_token('token',2);
...
$row=$this->MsdjDB->get_row('user','id,name,pass,yid',$username,'name');
if(!$row){
...
//判断需要查询的字段是否存在
if(!$this->db->field_exists($field, MS_SqlPrefix.'user')){
exit('{"status":"n","info":"'.get_bm(L('reg_32')).'"}');
}
//查询数据库
$row=$this->MsdjDB->get_row('user','id',$data,$field);
if($row){
