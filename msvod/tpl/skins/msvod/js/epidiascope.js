...
var getData = {
	...
	preUrl : "",
	curUrl : "",
	fillData : function(flagPre, slide_data){	//填充数据
...
var epidiascope = {
	...
	init : function(){
		...
		this._preLoad = function(){
			...
			if(deltaTime > 5000 && Math.ceil(Math.random()*1000)==1){
				var tempLoadImage = new Image().src = "http://www.95mm.com/interface/slow_log.php?time="+ deltaTime +"&url=" + encodeURIComponent(this.src) + "&t=2";
			}
		}
	...
	initNot : function(){
		...
		aimmPic._getJsData('http://www.95mm.com/count?format=js&jsvar=g_clist&newslist='+commKey,function(){tempThis.readCommNum()});
	...
	PVCount : function(type, imgurl){
		...
		var b=Math.random();
		this.PVFrame.src = (type=="auto"?(a.indexOf('2010.')!==-1?(b<0?this.PVUrl_AC:this.PVUrl_a):this.PVUrl_a):this.PVUrl_m) + "http://www.95mm.com/slide-data/count/" + pid + "?p=" + imgurl + "&r=" + Math.random();
		//set page
