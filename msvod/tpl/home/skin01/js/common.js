...
var home = {
        ...
        fav: function(uid,sid){
            ...
            if(!!otherDIds){
                $.getJSON(msvod_path+"index.php/home/ajax/favinit?did="+otherDIds+"&callback=?",function(res1){
                    followBtns.each(function(){
        ...
        fans: function(uid,sid){
            ...
            if(!!otherUserIds){
                $.getJSON(msvod_path+"index.php/home/ajax/fansinit?uid="+otherUserIds+"&callback=?",function(res1){
                    followBtns.each(function(){
...
function video_share(ac,title,url) {
	if(ac=='share_qzone'){
               var url="http://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url="+encodeURI(url)+"&title="+encodeURI('分享一部好看的视频，--《'+title+'》播放地址：');
        } else if(ac=='share_qwei'){
               var url="http://share.v.t.qq.com/index.php?c=share&a=index&appkey=&url="+encodeURI(url)+"&title="+encodeURI('分享一部好看的视频，--《'+title+'》播放地址：')+"&site=";
        } else if(ac=='share_weibo'){
               var url="http://service.weibo.com/share/share.php?appkey=&title="+encodeURI('分享一部好看的视频，--《'+title+'》播放地址：')+"&url="+encodeURI(url);
        } else if(ac=='share_baidu'){
               var url="http://tieba.baidu.com/f/commit/share/openShareApi?url="+encodeURI(url)+"&title="+encodeURI('分享一部好看的视频，--《'+title+'》播放地址：'+url)+"&desc="+encodeURI('分享一部好看的视频，--《'+title+'》播放地址：'+url)+"&comment="+encodeURI('分享一部好看的视频，--《'+title+'》播放地址：'+url);
        } else if(ac=='share_renren'){
               var url="http://widget.renren.com/dialog/share?resourceUrl="+encodeURI(url)+"&title="+encodeURI(title)+"&description="+encodeURI('分享一部好看的视频，--《'+title+'》播放地址：'+url)+"&srcUrl="+encodeURI(url);
        } else if(ac=='share_kaixin'){
               var url="http://www.kaixin001.com/rest/records.php?url="+encodeURI(url)+"&style=11&content="+encodeURI('分享一部好看的视频，--《'+title+'》播放地址：')+"&stime=&sig=";
	};
