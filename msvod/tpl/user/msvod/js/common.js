...
var pic=$('#pic').val();
$('#album_img').attr('src',msvod_path+'attachment/videotopic'+pic);
}
}); 
...
$.getJSON(msvod_path+"index.php/user/ajax/blog?neir="+$note.emotEditor("content")+"&callback=?",function(data) {
if(data['error']==1001){
do_alert('请输入您的说说内容！');
$note.emotEditor("focus");
return false;
...
$.getJSON(msvod_path+"index.php/user/ajax/blog_del?id="+id+"&callback=?",function(data) {
if(data['error']==1001){
do_alert('参数错误！');
return false;
}
...
dtpid=pid;
$.getJSON(msvod_path+"index.php/user/ajax/feed?dir="+dir+"&cid="+dtid+"&random="+Math.random()+"&callback=?",function(data) {
if(data['str']){
$("#feed").html(data['str']);
} else {
...
var msg = {
init:function(){
$.getJSON(msvod_path+"index.php/user/ajax/msg?random="+Math.random()+"&callback=?",function(data) {
if(data['nums']>0){
...
du:function(){
$.getJSON(msvod_path+"index.php/user/ajax/msg_du?random="+Math.random()+"&callback=?",function(data) {
if(data['error']==1000){
do_alert('您登录已经超时！');
return false;
} else{
...
del:function(id){
if(window.confirm('你确定要删除吗？')){
$.getJSON(msvod_path+"index.php/user/ajax/msg_del?id="+id+"&random="+Math.random()+"&callback=?",function(data) {
if(data['error']==1001){
do_alert('参数错误！');
return false;
} else if(data['error']==1000){
...
$.getJSON(msvod_path+"index.php/user/ajax/gbook_hf?fid="+id+"&uida="+uida+"&neir="+neir+"&random="+Math.random()+"&callback=?",function(data) {
if(data['error']==1001){
do_alert('参数错误！');
return false;
} else if(data['error']==1000){
...
if(window.confirm('你确定要删除吗？')){
$.getJSON(msvod_path+"index.php/user/ajax/gbook_del?id="+id+"&random="+Math.random()+"&callback=?",function(data) {
if(data['error']==1001){
do_alert('参数错误！');
return false;
} else if(data['error']==1000){
...
$.getJSON(msvod_path+"index.php/user/ajax/"+type+"_del?id="+id+"&random="+Math.random()+"&callback=?",function(data) {
if(data['error']==1001){
do_alert('参数错误！');
return false;
} else if(data['error']==1000){
...
if(ok){
$.getJSON(msvod_path+"index.php/user/ajax/web?dir="+dir+"&random="+Math.random()+"&callback=?",function(data) {
if(data['error']==1001){
do_alert('参数错误！');
return false;
} else if(data['error']==1000){
...
//签到
function qiandao(){
$.getJSON(msvod_path+"index.php/user/ajax/qiandao?random="+Math.random()+"&callback=?",function(data) {
if(data['error']==1001){
do_alert('您今天已经签到，请明天继续！');
$('.msvod_qd span').html('今天已签到');
return false;
} else if(data['error']==1000){
do_alert('您登录已经超时！');
return false;
...
$.getJSON(msvod_path+"index.php/user/ajax/open_del?cid="+cid+"&random="+Math.random()+"&callback=?",function(data) {
if(data['error']==1000){
do_alert('您登录已经超时！');
return false;
} else{
...
var url=msvod_path+"index.php/"+type+"/search?key="+encodeURIComponent(key);
window.open(url);
}
}
//发送手机验证码
