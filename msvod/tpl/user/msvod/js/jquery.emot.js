...
(function($) {
	var emotRegStr = "<img([^\<\>(src=)])*src=\""+ msvod_path +"packs/images/faces/e([0-9]{0,}).gif\"([^\<\>])*>";
	...
	var methods = {
		init : function(options) {
			...
			if(options.emot){
				...
				for (i = 1; i <= 56; i++) {
					emotHtml += '<li emotId=' + i + '><img src="'+ msvod_path +'packs/images/faces/e' + i + '.gif" class=\"emot_img\" /></li>'
				}
				emotHtml += '</ul>';
			}
							...
							if($emotMenu==null){
								...
								.find("ul>li").click(function(e) {
									...
									if(isPad){
										...
										var sHtml = "&nbsp;<img src=\""+ msvod_path +"packs/images/faces/e"+id+".gif\">&nbsp;";
										if(!objRange['range'] || $obj.is(":hidden") == true){
											...
											if(browser.ie){
												if(!objRange['range'].length==1){
													...
													objRange['range'](0).src = msvod_path +"packs/images/faces/e"+id+".gif";
													objRange['range'].select();
												}
											}else{
